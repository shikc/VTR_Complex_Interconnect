...
constexpr std::array<const char*, NUM_SIDES+1> SIDE_STRING = {{"TOP", "RIGHT", "BOTTOM", "LEFT", "NUM_SIDES"}}; //String versions of side orientations
...
struct t_type_descriptor /* TODO rename this.  maybe physical type descriptor or complex logic block or physical logic block*/
{
    ...
    t_fastpath fp; //added by shikc in 191218
...
enum class SwitchType {
    ...
    UNISHORT,  //A non-configurable electricallly shorted connection ( only used in the uniconneciton)
...
/* Lists all the important information about a detail connection.     *
 * [0 .. det_routing_arch.num_detail]                                 *
 */
struct t_detail_loc {
    int x;
    int y;
    ...
    bool operator==(t_detail_loc rhs){
        return (this->x==rhs.x && this->y==rhs.y && this->z==rhs.z && this->tn==rhs.tn && this->side==rhs.side);
    }
};
/*
typedef enum e_detail_type : unsigned char{
    OPIN=0,
    IPIN,
    CHANX,
    CHANY,
...
*/
struct t_detailInterConnect_inf {
    char* from_type;
    char* to_type;
    char* from_pt;
    char* to_pt;
...
struct t_wire_switchpoints {
    ...
    bool operator==(const t_wire_switchpoints& obj) const {
        return segment_name == obj.segment_name;//one segment only have one type switchpoints, so we only compare segment
    }
};
...
struct t_detail_from_inf;
struct t_gsb_todir_group_inf;

struct t_common_from_inf {
    ...
    std::vector<int> from_switchpoints;

    std::vector<std::string> pin_types;

    virtual bool only_detail() const {return false;}
    virtual const std::vector<t_detail_from_inf> detail_inf() const {return {};}
    virtual const std::vector<int> to_fix_order() const {return {};}
    virtual int overlap() const {return 0;}

    virtual bool only_use_group() const {return false;}
    virtual const std::vector<t_gsb_todir_group_inf> gsb_todir_group_inf() const {return {};}

    virtual ~t_common_from_inf(){}
};

//for omux
struct t_omux_inf {
    std::string name;
    std::vector<std::string> pbtype_names;
    int mux_nums;
    int total_froms;
    int num_foreach;
    short arch_omux_switch;
    int offset;
};
...
struct t_detail_from_inf {
	...
	int to_input;
    int num_foreach;
    std::vector<std::string> from_detail;
	//std::string lut_name;//for clb output to gsb
	//std::string pin_type;
};

struct t_imux_from_inf : t_common_from_inf {
	bool _only_detail;
    std::vector<t_detail_from_inf> _imux_detail_inf;
	std::vector<int> _to_fix_order;
    int _overlap;

    bool only_detail() const { return _only_detail;}
    const std::vector<t_detail_from_inf> detail_inf() const { return _imux_detail_inf;}
    const std::vector<int> to_fix_order() const { return _to_fix_order;}
...
struct t_mux_same_from_inf {
    int to_track_index; //index of to_track
    e_from_type mux_from_type;
    std::string from_name;

    //std::vector<std::string> from_details;
    std::vector<e_side> from_sides;
    std::vector<std::string> from_pin_types;
    std::vector<int> from_inds;
...
struct t_imux_group_inf {
	std::vector<std::string> lut_name;
	int mux_nums_eachlut;
    std::vector<t_imux_from_inf> imux_from_inf;
};

struct t_firststage_mux_inf {
    std::string mux_name;
    //int medium_id;
    std::vector<t_mux_same_from_inf> from_infs;
    std::vector<std::string> firststage_mux_name;
    int to_ind;
    e_side to_side;
...
struct t_secondstage_mux_inf : t_firststage_mux_inf {
    std::vector<std::string> firststage_mux_name;
    int to_ind;
    e_side to_side;
    std::string to_pin_name;
...
struct t_imux_inf {
    ...
    std::vector<std::string> pbtype_names;
	int lut_group;
    short arch_imux_switch;
	std::vector<t_imux_group_inf> imux_group_inf;

    bool twostage_mux_spec;
    int first_stage_switch_id;
    std::vector<t_firststage_mux_inf> first_stages;
    std::vector<t_secondstage_mux_inf> second_stages;
};

//gsb
struct t_gsb_todir_group_inf : t_common_from_inf {
	e_seg_dir to_dir;
    std::vector<e_seg_dir> from_dir;
	int actual_start;
	int actuall_foreach;

    ~t_gsb_todir_group_inf(){}
};

struct t_gsb_from_inf : t_common_from_inf {
    bool _only_detail;
	std::vector<t_detail_from_inf> _gsb_detail_inf;
	//std::vector<int> to_fix_order;
	
    bool _only_use_group;
	std::vector<t_gsb_todir_group_inf> _gsb_todir_group_inf;

    bool only_detail() const { return _only_detail;}
    const std::vector<t_detail_from_inf> detail_inf() const { return _gsb_detail_inf;}

    bool only_use_group() const { return _only_use_group;}
    const std::vector<t_gsb_todir_group_inf> gsb_todir_group_inf() const { return _gsb_todir_group_inf;}

    ~t_gsb_from_inf(){}
};

struct t_gsb_group_inf {
	std::vector<std::string> seg_name;
	int track_nums;
    std::vector<t_gsb_from_inf> gsb_from_inf;
};

struct t_gsb_multistage_mux_inf {
    //std::vector<std::tuple<std::string, std::string>> to_track;//[<to_seg_name, detail_track>...]
    std::vector<std::string> to_seg_names;
    std::vector<std::pair<e_side, int>> to_tracks;

    std::vector<t_mux_same_from_inf> reuse_mux_inf;
    std::vector<std::vector<t_mux_same_from_inf>> normal_mux_infs;
...
struct t_gsb_inf {
    ...
    std::vector<std::string> pbtype_names;
	int gsb_seg_group;
	std::vector<t_gsb_group_inf> gsb_group_inf;
    std::vector<t_gsb_multistage_mux_inf> gsb_multistage_mux_inf;

    bool twostage_mux_spec;
    int arch_gsb_switch_id;
    std::vector<t_firststage_mux_inf> first_stages;
    std::vector<t_firststage_mux_inf> second_stages;
};
...
struct t_arch {
    ...
    int num_directs = 0;
    t_detailInterConnect_inf* DetailICs = nullptr;
    int num_detailics = 0;
    ...
    //std::vector<t_imux_inf> imux_inf;
    //std::vector<t_omux_inf> omux_inf;
    std::vector<t_gsb_inf> gsb_inf;
