...
void free_arch(t_arch* arch) {
    ...
    for (int i = 0; i < arch->num_detailics; ++i) {
        vtr::free(arch->DetailICs[i].from_type);
        vtr::free(arch->DetailICs[i].to_type);
        vtr::free(arch->DetailICs[i].from_pt);
        vtr::free(arch->DetailICs[i].to_pt);
    }
    vtr::free(arch->DetailICs);
