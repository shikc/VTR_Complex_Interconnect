...
/* Function prototypes */
/*   Populate data */
std::vector<std::string> split_fc_value(std::string s,char ch);
...
//static void Process_FastPath(pugi::xml_node Node, t_physical_tile_type* PhysicalTileType, const pugiutil::loc_data& loc_data);   //added by shikc in 191217
...
static void ProcessDetailICs(pugi::xml_node Parent, t_detailInterConnect_inf** DetailICs, int* NumDetailICs, const t_arch_switch_inf* Switches, const int NumSwitches, const pugiutil::loc_data& loc_data);
...
/* Processes the gsb_arch from the xml architecture file. */

static void ProcessFromDetail(pugi::xml_node Detail_Node, std::vector<t_detail_from_inf>& detail_fs, const pugiutil::loc_data& loc_data);
static void ProcessFirstStage(pugi::xml_node Stage_Node, std::vector<t_firststage_mux_inf>& first_stages, const pugiutil::loc_data& loc_data);
static void ProcessSecondStage(pugi::xml_node Stage_Node, std::vector<t_secondstage_mux_inf>& second_stages, const pugiutil::loc_data& loc_data);



static void ProcessGsbMultistageMux(pugi::xml_node Multimux_Node, t_gsb_multistage_mux_inf& multistage_mux_inf, const pugiutil::loc_data& loc_data);
static void ProcessGSB(pugi::xml_node Gsb_Node, t_arch* arch, const pugiutil::loc_data& loc_data);
static void ProcessGSBarchs(pugi::xml_node Parent, t_arch* arch, const pugiutil::loc_data& loc_data);
...
e_from_type string_to_fromtype(std::string fromtype_str);
e_side toString_to_side(char to_string);
e_side fromString_to_side(char from_string);
/*
 *
 *
...
void XmlReadArch(const char* ArchFile, const bool timing_enabled, t_arch* arch, t_type_descriptor** Types, int* NumTypes) {
    ...
    try {
        ...
        Next = get_single_child(architecture, "gsb_arch", loc_data, OPTIONAL);
        if (Next) {
            ProcessGSBarchs(Next, arch, loc_data);
        }
        ...
        /* Process detailInterconnect */
        Next = get_single_child(architecture, "detailiclist", loc_data, OPTIONAL);
        if (Next) {
            ProcessDetailICs(Next, &(arch->DetailICs), &(arch->num_detailics),
                           arch->Switches, arch->num_switches,
                           loc_data);
        }
...
static void Process_Fc_Values(pugi::xml_node Node, t_default_fc_spec& spec, const pugiutil::loc_data& loc_data) {
    ...
    std::vector<std::string> in_val_string=split_fc_value(in_val_attrib.value(), ' ');
    //spec.in_value = vtr::atof(in_val_attrib.value());  added by shikc in 191217
    for(int i=0; i<4; i++){
        spec.in_value[i] = vtr::atof(in_val_string[i]);
    }
    ...
    std::vector<std::string> out_val_string=split_fc_value(out_val_attrib.value(), ' ');
    //spec.out_value = vtr::atof(out_val_attrib.value());
    for(int i=0; i<4; i++){
        spec.out_value[i] = vtr::atof(out_val_string[i]);
    }
}
/*
//added by shikc in 191217
static void Process_FastPath(pugi::xml_node Node, t_physical_tile_type* PhysicalTileType, const pugiutil::loc_data& loc_data){
    auto fastpath_c_attrib = get_attribute(Node, "Fc_output_to_clb_input", loc_data);
    PhysicalTileType->fp.fc_clb_output=vtr::atof(fastpath_c_attrib.value());

    auto fastpath_p_attrib = get_attribute(Node, "Fc_output_to_pad_input", loc_data);
    PhysicalTileType->fp.fc_pad_output=vtr::atof(fastpath_p_attrib.value());
}
*/
            ...
            for (int iport = 0; iport < pb_type->num_ports; ++iport) {
                ...
                for (const auto& fc_override : fc_overrides) {
                    ...
                    if (apply_override) {
                        ...
                        for(int i=0; i<4; i++)
                            fc_spec.fc_value[i] = fc_override.fc_value[i];  //added by shikc in 191217
...
static t_fc_override Process_Fc_override(pugi::xml_node node, const pugiutil::loc_data& loc_data) {
    ...
    for (auto attrib : node.attributes()) {
        if (attrib.name() == std::string("port_name")) {
            ...
            //fc_override.fc_value = vtr::atof(attrib.value()); added by shikc in 191217
            std::vector<std::string> fc_val_string=split_fc_value(attrib.value(), ' ');
            //spec.in_value = vtr::atof(in_val_attrib.value());
            for(int i=0; i<4; i++)
                fc_override.fc_value[i] = vtr::atof(fc_val_string[i]);
            seen_fc_value = true;
        } else {
            archfpga_throw(loc_data.filename_c_str(), loc_data.line(node),
...
static void ProcessSegments(pugi::xml_node Parent,
    ...
    for (i = 0; i < NumSegs; ++i) {
        ...
        //Segs[i].is_omux_medium = get_attribute(Node, "is_omux_medium", loc_data, OPTIONAL).as_bool(0);
        //Segs[i].is_imux_medium = get_attribute(Node, "is_imux_medium", loc_data, OPTIONAL).as_bool(0);
        ...
        if (0 == strcmp(tmp, "bidir")) {
            ...
            // bend list only tested in the unidir architecture
...
static void ProcessFromDetail(pugi::xml_node Detail_Node, std::vector<t_detail_from_inf>& detail_fs, const pugiutil::loc_data& loc_data) {
    const char* tmp;
    t_detail_from_inf df;

    bool is_to_seg = false;
    std::vector<e_seg_dir> to_dirs;
    std::vector<std::string> from_details;
    std::vector<int> to_inputs;

    tmp = get_attribute(Detail_Node, "from_detail", loc_data).as_string(nullptr);
    if (tmp) {
        auto df_strs = vtr::split(tmp, " ");
        for (const auto& df_str : df_strs) {
                from_details.push_back(df_str);  
        }
    }
    else {
        archfpga_throw(loc_data.filename_c_str(), loc_data.line(Detail_Node),
                        "No from_detail specified for the detail inf.\n");
    }

    tmp = get_attribute(Detail_Node, "to_dir", loc_data, OPTIONAL).as_string(nullptr);
    if (tmp) {
        is_to_seg=true;
        auto to_dir_strs = vtr::split(tmp, " ");
        for (const auto& to_dir_str : to_dir_strs) {
            to_dirs.push_back(string_to_segdir(to_dir_str));   
        }
    }
    
    ReqOpt to_track_req = ReqOpt::REQUIRED;
    tmp = get_attribute(Detail_Node, "to_input", loc_data, OPTIONAL).as_string(nullptr);
    if (tmp) {
        auto to_input_strs = vtr::split(tmp, " ");
        for (const auto& to_input_str : to_input_strs) {
            to_inputs.push_back(vtr::atoi(to_input_str));   
        }
        to_track_req = ReqOpt::OPTIONAL;
    }

    tmp = get_attribute(Detail_Node, "to_track", loc_data, to_track_req).as_string(nullptr);
    if (tmp) {
        auto to_input_strs = vtr::split(tmp, " ");
        for (const auto& to_input_str : to_input_strs) {
            to_inputs.push_back(vtr::atoi(to_input_str));   
    ...
    df.num_foreach = get_attribute(Detail_Node, "num_foreach", loc_data, OPTIONAL).as_int(1);

    VTR_ASSERT(to_inputs.size()*df.num_foreach == from_details.size());

    //待完善
    /*VTR_LOG("to_inputs:");
    for(auto in : to_inputs){
        VTR_LOG("%d\t", in);
    }*/
    if(is_to_seg){
        for(auto it = to_dirs.begin();it!=to_dirs.end();it++){
            for(auto it2 = from_details.begin();it2!=from_details.end();){
                std::vector<std::string> fds_each;
                for(auto in = 0; in < df.num_foreach; in++){
                    fds_each.push_back(*it2);
                    it2++;
                }
                VTR_ASSERT(it2<=from_details.end());
                df.from_detail = fds_each;
                df.to_dir=*it;
                df.to_input=to_inputs[(it2-from_details.begin())/df.num_foreach-1];
                //VTR_LOG("to_inputs_index:%d\t",(it2-from_details.begin())/df.num_foreach);
                detail_fs.push_back(df);
            }
        }
    }
    else{
        for(auto it2 = from_details.begin();it2!=from_details.end();){
            ...
            VTR_ASSERT(it2<=from_details.end());
            df.from_detail = fds_each;
            df.to_dir=e_seg_dir::NUM_D_TYPES;
            df.to_input=to_inputs[(it2-from_details.begin())/df.num_foreach-1];
            detail_fs.push_back(df);
        }
    }
...
static void ProcessFirstStage(pugi::xml_node Stage_Node, std::vector<t_firststage_mux_inf>& first_stages, const pugiutil::loc_data& loc_data) {
    ...
    for (int i_m = 0; i_m < num_mux; i_m++) {
        t_firststage_mux_inf fm;
        fm.mux_name = get_attribute(Node, "name", loc_data).as_string();
        


        num_froms = count_children(Node, "from", loc_data);
        fm.from_infs.reserve(num_froms);
        SubElem = get_first_child(Node, "from", loc_data);
        for (int i_f = 0; i_f < num_froms; i_f++) {
            tmp = get_attribute(SubElem, "mux_name", loc_data, OPTIONAL).as_string(nullptr);
            if(tmp){
                auto tmp_mux_name = vtr::split(tmp, " ");

                for(auto i: tmp_mux_name){
                    fm.firststage_mux_name.push_back(i);
                }
                SubElem = SubElem.next_sibling(SubElem.name());
                continue;
            }
            t_mux_same_from_inf mux_same_f;
            mux_same_f.to_track_index = -1;

            tmp = get_attribute(SubElem, "type", loc_data).as_string();
            mux_same_f.mux_from_type = string_to_fromtype(tmp);
            mux_same_f.from_name = get_attribute(SubElem, "name", loc_data).as_string();
            ...
            for (auto const& detail_str : detail_strs) {
                if (mux_same_f.mux_from_type == e_from_type::FT_SEGMENT) {
                    mux_same_f.from_sides.push_back(fromString_to_side(detail_str[0]));
                    mux_same_f.from_inds.push_back(std::stoi(detail_str.substr(1, detail_str.length() - 1)));
                    mux_same_f.from_pin_types.push_back("");
                }
                else if (mux_same_f.mux_from_type == e_from_type::FT_PB) {
                    mux_same_f.from_sides.push_back(NUM_SIDES);
                    auto pin_strs = vtr::split(detail_str, ":");
                    mux_same_f.from_inds.push_back(std::stoi(pin_strs[1]));
                    mux_same_f.from_pin_types.push_back(pin_strs[0]);
                } else if(mux_same_f.mux_from_type == e_from_type::FT_IMUX){
                    mux_same_f.from_sides.push_back(NUM_SIDES);
                    auto pin_strs = vtr::split(detail_str, ":");
                    mux_same_f.from_inds.push_back(std::stoi(pin_strs[1]));
                    mux_same_f.from_pin_types.push_back(pin_strs[0]);
            ...
            fm.from_infs.push_back(mux_same_f);
            SubElem = SubElem.next_sibling(SubElem.name());
        }

        first_stages.push_back(fm);
        Node = Node.next_sibling(Node.name());
    }
    return;
}

static void ProcessSecondStage(pugi::xml_node Stage_Node, std::vector<t_firststage_mux_inf>& second_stages, const pugiutil::loc_data& loc_data) {
    pugi::xml_node Node;
    pugi::xml_node SubElem;
    const char* tmp;
    std::string tmp2;
    ...
    Node = get_first_child(Stage_Node, "mux", loc_data);
    for (int i_m = 0; i_m < num_mux; i_m++) {
        t_firststage_mux_inf sm;
        t_firststage_mux_inf fm;
        sm.mux_name = get_attribute(Node, "name", loc_data).as_string();

        
        tmp = get_attribute(Node, "to_pin", loc_data, OPTIONAL).as_string(nullptr);
        if(tmp){
            is_imux = true;
            sm.to_side = NUM_SIDES;
            auto pin_strs = vtr::split(tmp, ":");
            sm.to_ind = std::stoi(pin_strs[1]);
            sm.to_pin_name = pin_strs[0];
        }

        else {
            sm.to_name = get_attribute(Node, "to_seg_name", loc_data).as_string();
            tmp2 = get_attribute(Node, "to_track", loc_data).as_string();
            sm.to_side = toString_to_side(tmp2[0]);
            sm.to_ind = std::stoi(tmp2.substr(1, tmp2.length() - 1));
            sm.to_pin_name = "";
        }

        num_froms = count_children(Node, "from", loc_data);
        sm.from_infs.reserve(num_froms);
        SubElem = get_first_child(Node, "from", loc_data);
        for (int i_f = 0; i_f < num_froms; i_f++) {
            tmp = get_attribute(SubElem, "mux_name", loc_data, OPTIONAL).as_string(nullptr);
            if(tmp){
                ...
                for(auto i: tmp_mux_name){
                    sm.firststage_mux_name.push_back(i);
                }
                SubElem = SubElem.next_sibling(SubElem.name());
	    ...
	    mux_same_f.switchpoint = get_attribute(SubElem, "switchpoint", loc_data, OPTIONAL).as_int(0);
            ...
            for (auto const& detail_str : detail_strs) {
                ...
                //mux_same_f.from_details.push_back(detail_str);
            }

            sm.from_infs.push_back(mux_same_f);
            SubElem = SubElem.next_sibling(SubElem.name());
        }

        second_stages.push_back(sm);
        Node = Node.next_sibling(Node.name());
    }
    return;
}
...
static void ProcessGsbMultistageMux(pugi::xml_node Multimux_Node, t_gsb_multistage_mux_inf& multistage_mux_inf, const pugiutil::loc_data& loc_data) {
    ...
    tmp = get_attribute(Multimux_Node, "to_seg_name", loc_data).as_string(nullptr);
    tmp2 = get_attribute(Multimux_Node, "to_track", loc_data).as_string(nullptr);
    if (tmp && tmp2) {
        auto to_seg_strs = vtr::split(tmp, " ");
        auto to_track_strs = vtr::split(tmp2, " ");
        VTR_ASSERT(to_seg_strs.size()==to_track_strs.size());
        for (int i_str = 0; i_str < to_seg_strs.size(); i_str++) {
            multistage_mux_inf.to_seg_names.push_back(to_seg_strs[i_str]);
            std::string to_track_str=to_track_strs[i_str];

            e_side to_side = toString_to_side(to_track_str[0]);
            int to_track = vtr::atoi(to_track_str.substr(1, to_track_str.length()-1));
            multistage_mux_inf.to_tracks.push_back(std::make_pair(to_side, to_track));
        }
    }

    expect_only_children(Multimux_Node, {"reuse_mux", "normal_mux"}, loc_data);

    //parse reuse mux inf
    Node = get_single_child(Multimux_Node, "reuse_mux", loc_data);
    expect_only_children(Node, {"from"}, loc_data);
    num_froms = count_children(Node, "from", loc_data);
    multistage_mux_inf.reuse_mux_inf.reserve(num_froms);
    SubElem = get_first_child(Node, "from", loc_data);
    for (int i_f = 0; i_f < num_froms; i_f++){
        t_mux_same_from_inf mux_same_f;
        tmp = get_attribute(SubElem, "type", loc_data).as_string();
        ...
        mux_same_f.from_sides.reserve(detail_strs.size());
        mux_same_f.from_pin_types.reserve(detail_strs.size());
        mux_same_f.from_inds.reserve(detail_strs.size());
        ...
        for(auto const& detail_str : detail_strs){
            if(mux_same_f.mux_from_type == e_from_type::FT_SEGMENT){
                ...
                mux_same_f.from_inds.push_back(std::stoi(detail_str.substr(1,detail_str.length()-1)));
                mux_same_f.from_pin_types.push_back("");
            } else if (mux_same_f.mux_from_type == e_from_type::FT_OMUX) {
                mux_same_f.from_sides.push_back(NUM_SIDES);
                mux_same_f.from_inds.push_back(std::stoi(detail_str));
                mux_same_f.from_pin_types.push_back("");
            } else if (mux_same_f.mux_from_type == e_from_type::FT_PB) {
                mux_same_f.from_sides.push_back(NUM_SIDES);
                auto pin_strs = vtr::split(detail_str, ":");
        ...
        mux_same_f.to_track_index = -1;
        multistage_mux_inf.reuse_mux_inf.push_back(mux_same_f);
        SubElem = SubElem.next_sibling(SubElem.name());
    }

    //parse normal mux inf
    int num_normal_muxs = count_children(Multimux_Node, "normal_mux", loc_data);
    multistage_mux_inf.normal_mux_infs.reserve(num_normal_muxs);
    Node = get_first_child(Multimux_Node, "normal_mux", loc_data);
    for (int i_n = 0; i_n < num_normal_muxs; i_n++){
        std::vector<t_mux_same_from_inf> mux_same_fs;
        int to_track_index = get_attribute(Node, "to_track_index", loc_data).as_int();
        num_froms = count_children(Node, "from", loc_data);
        mux_same_fs.reserve(num_froms);
        SubElem = get_first_child(Node, "from", loc_data);
        for (int i_f = 0; i_f < num_froms; i_f++){
            t_mux_same_from_inf mux_same_f;
            mux_same_f.to_track_index = to_track_index;
            ...
            mux_same_fs.push_back(mux_same_f);
            SubElem = SubElem.next_sibling(SubElem.name());
        }

        multistage_mux_inf.normal_mux_infs.push_back(mux_same_fs);
        Node = Node.next_sibling(Node.name());
    }
...
static void ProcessGSB(pugi::xml_node Gsb_Node, t_arch* arch, const pugiutil::loc_data& loc_data) {
    ...
    tmp = get_attribute(Gsb_Node, "name", loc_data).as_string(nullptr);
    if (tmp) {
        gsb.name = tmp;
    }
    else {
        archfpga_throw(loc_data.filename_c_str(), loc_data.line(Gsb_Node),
                        "No name specified for the gsb.\n");
    }

    tmp = get_attribute(Gsb_Node, "pbtype_name", loc_data).as_string(nullptr);
    if (tmp) {
        gsb.pbtype_names = vtr::split(tmp, " ");
    }
    else {
        archfpga_throw(loc_data.filename_c_str(), loc_data.line(Gsb_Node),
                        "No pb_type specified for the gsb.\n");
    }

    tmp = get_attribute(Gsb_Node, "arch_gsb_switch", loc_data).as_string(nullptr);
    if (tmp) {
        /* Match names */
        int j;
        int NumSwitches = arch->num_switches;
        ...
        gsb.arch_gsb_switch_id = j;
    }
    else {
        archfpga_throw(loc_data.filename_c_str(), loc_data.line(Gsb_Node),
                       "No arch_omux_switch specified\n");
    }
    
    gsb.gsb_seg_group = get_attribute(Gsb_Node, "gsb_seg_group", loc_data, OPTIONAL).as_int(1);

    expect_only_children(Gsb_Node, {"seg_group", "multistage_muxs"}, loc_data);

    int num_groups = count_children(Gsb_Node, "seg_group", loc_data);
    VTR_ASSERT(gsb.gsb_seg_group == num_groups);
    gsb.gsb_group_inf.reserve(num_groups);
    Node = get_first_child(Gsb_Node, "seg_group", loc_data);
    for (int i_g = 0; i_g < num_groups; i_g++) {
        /* use a temp variable which will be assigned to omux later */
        t_gsb_group_inf gsb_g;

        /* get name */
        tmp = get_attribute(Node, "name", loc_data).as_string(nullptr);
        if (tmp) {
            gsb_g.seg_name = vtr::split( tmp , " ");
        }
        else {
            archfpga_throw(loc_data.filename_c_str(), loc_data.line(Node),
                            "No name specified for the gsb seg group #%d.\n", i_g);
        }

        itmp = get_attribute(Node, "track_nums", loc_data).as_int();
        if (itmp) {
            gsb_g.track_nums = itmp;
           
        }
        else {
            archfpga_throw(loc_data.filename_c_str(), loc_data.line(Node),
                            "No track_nums specified for the gsb seg group #%d.\n", i_g);
        }
        

        /* assign the imx_g to the imux_group_inf vector */
        gsb.gsb_group_inf.push_back(gsb_g);

        Node = Node.next_sibling(Node.name());
    }


    Node = get_single_child(Gsb_Node, "multistage_muxs", loc_data);
    if(Node) {

        gsb.twostage_mux_spec = true;
        SubElem = get_single_child(Node, "first_stage", loc_data);
        ProcessFirstStage(SubElem, gsb.first_stages, loc_data);
        SubElem = get_single_child(Node, "second_stage", loc_data);
        ProcessSecondStage(SubElem, gsb.second_stages, loc_data);

        
    }
    else{
        gsb.twostage_mux_spec = false;
    }
    arch->gsb_inf.push_back(gsb);
    return;
}
...
static void ProcessGSBarchs(pugi::xml_node Parent, t_arch* arch, const pugiutil::loc_data& loc_data) {
    ...
    int num_gsbs = count_children(Parent, "gsb", loc_data);
    arch->gsb_inf.reserve(num_gsbs);
    Node = get_first_child(Parent, "gsb", loc_data);
    for (int i_gsb = 0; i_gsb < num_gsbs; i_gsb++) {
        ProcessGSB(Node, arch, loc_data);
        Node = Node.next_sibling(Node.name());
    }
...
static void ProcessBend(pugi::xml_node Node, std::vector<int>& list, std::vector<int>& part_len, bool& isbend, const int len, const pugiutil::loc_data& loc_data) {
    ...
    for(size_t i_len = 0; i_len < list.size(); i_len++){
        if (list[i_len] == 0) {
            ...
            VTR_ASSERT(tmp_len < list.size()+1);
...
static void ProcessDetailICs(pugi::xml_node Parent, t_detailInterConnect_inf** DetailICs, int* NumDetailICs, const t_arch_switch_inf* Switches, const int NumSwitches, const pugiutil::loc_data& loc_data) {
    ...
    const char* from_pt_name;
    const char* to_pt_name;
    ...
    /* Count the children and check they are detail connections */
    expect_only_children(Parent, {"detailic"}, loc_data);
    *NumDetailICs = count_children(Parent, "detailic", loc_data);

    /* Alloc detail list */
    *DetailICs = nullptr;
    if (*NumDetailICs > 0) {
        *DetailICs = (t_detailInterConnect_inf*)vtr::malloc(*NumDetailICs * sizeof(t_detailInterConnect_inf));
        memset(*DetailICs, 0, (*NumDetailICs * sizeof(t_detailInterConnect_inf)));
    }

    /* Load the details. */
    Node = get_first_child(Parent, "detailic", loc_data);
    for (i = 0; i < *NumDetailICs; ++i) {
        expect_only_attributes(Node, {"from_type", "from_pt", "from_x", "from_y", "from_z", "from_tn", "to_type", "to_pt", "to_x", "to_y","to_z", "to_tn", "switch_name", "from_side", "to_side"}, loc_data);
        ...
        */
        ...
        from_pt_name = get_attribute(Node, "from_pt", loc_data).value();
        to_pt_name = get_attribute(Node, "to_pt", loc_data).value();
        from_type = get_attribute(Node, "from_type", loc_data).value();
        to_type = get_attribute(Node, "to_type", loc_data).value();

        if (0 == strcmp(from_type, "ipin") || 0 == strcmp(to_type, "opin") ) {
            archfpga_throw(loc_data.filename_c_str(), loc_data.line(Node),
                           "The source pin/sink pin are error type: %s %s.\n",
                           from_type, to_type);
        }
        /* Check that to_pin and the from_pin are not the same */
        if (0 == strcmp(to_pt_name, from_pt_name) && 0 == strcmp(from_type, "opin") ) {
            archfpga_throw(loc_data.filename_c_str(), loc_data.line(Node),
                           "The source pin and sink pin are the same: %s.\n",
                           to_pt_name);
        }
        (*DetailICs)[i].from_pt = vtr::strdup(from_pt_name);
        (*DetailICs)[i].to_pt = vtr::strdup(to_pt_name);
        (*DetailICs)[i].from_type = vtr::strdup(from_type);
        (*DetailICs)[i].to_type = vtr::strdup(to_type);


        (*DetailICs)[i].from.x = get_attribute(Node, "from_x", loc_data).as_int(0);
        (*DetailICs)[i].from.y = get_attribute(Node, "from_y", loc_data).as_int(0);
        (*DetailICs)[i].from.z = get_attribute(Node, "from_z", loc_data).as_int(0);
        (*DetailICs)[i].from.tn = get_attribute(Node, "from_tn", loc_data, OPTIONAL).as_int(0);

        (*DetailICs)[i].to.x = get_attribute(Node, "to_x", loc_data).as_int(0);
        (*DetailICs)[i].to.y = get_attribute(Node, "to_y", loc_data).as_int(0);
        (*DetailICs)[i].to.z = get_attribute(Node, "to_z", loc_data).as_int(0);
        (*DetailICs)[i].to.tn = get_attribute(Node, "to_tn", loc_data, OPTIONAL).as_int(0);

        std::string from_side_str = get_attribute(Node, "from_side", loc_data, OPTIONAL).value();
        (*DetailICs)[i].from.side = string_to_side(from_side_str);
        std::string to_side_str = get_attribute(Node, "to_side", loc_data, OPTIONAL).value();
        (*DetailICs)[i].to.side = string_to_side(to_side_str);
        ...
        if (switch_name != nullptr) {
            ...
            (*DetailICs)[i].switch_type = j; //Save the correct switch index
        } else {
...
e_from_type string_to_fromtype(std::string fromtype_str) {
    e_from_type fromtype = e_from_type::NUM_FT_TYPES;
    if (fromtype_str.empty()) {
        fromtype = e_from_type::NUM_FT_TYPES;
    } else if (fromtype_str == "seg") {
        fromtype = e_from_type::FT_SEGMENT;
    }  else if (fromtype_str == "pb") {
        fromtype = e_from_type::FT_PB;
    } else if (fromtype_str == "imux") {
        fromtype = e_from_type::FT_IMUX;
    } else {
        archfpga_throw(__FILE__, __LINE__,
                       "Invalid from_type specification");
    }
    return fromtype;
}

e_seg_dir string_to_segdir(std::string segdir_str) {
    e_seg_dir segdir = e_seg_dir::NUM_D_TYPES;
    if (segdir_str.empty()) {
        segdir = e_seg_dir::NUM_D_TYPES;
    } else if (segdir_str == "E") {
        segdir = e_seg_dir::D_E;
    } else if (segdir_str == "W") {
        segdir = e_seg_dir::D_W;
    } else if (segdir_str == "S") {
        segdir = e_seg_dir::D_S;
    } else if (segdir_str == "N") {
        segdir = e_seg_dir::D_N;
    } else {
        archfpga_throw(__FILE__, __LINE__,
                       "Invalid seg_dir specification");
    }
    return segdir;
}
...
e_side toString_to_side(char to_string){
    if (to_string == 'E') {
        ...
        archfpga_throw(__FILE__, __LINE__,
                       "Invalid to_string specification for multistage mux");
    }
}
...
e_side fromString_to_side(char from_string){
    if (from_string == 'E') {
        return LEFT;
    } else if (from_string == 'W') {
        return RIGHT;
    } else if (from_string == 'S') {
        return TOP;
    } else if (from_string == 'N') {
        return BOTTOM;
    } else {
        archfpga_throw(__FILE__, __LINE__,
                       "Invalid to_string specification for multistage mux");
