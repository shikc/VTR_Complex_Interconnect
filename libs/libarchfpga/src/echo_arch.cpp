...
void EchoArch(const char* EchoFile, const t_type_descriptor* Types, const int NumTypes, const t_arch* arch) {
    ...
    for (i = 0; i < NumTypes; ++i) {
        ...
        for (const t_fc_specification& fc_spec : Types[i].fc_specs) {
            ...
            fprintf(Echo, " fc_value: %f %f %f %f", fc_spec.fc_value[0],fc_spec.fc_value[1],fc_spec.fc_value[2],fc_spec.fc_value[3]);
...
void PrintArchInfo(FILE* Echo, const t_arch* arch) {
    ...
    //print gsb arch information
//    fprintf(Echo, "imux information:\n");
//    for(auto const& imi : arch->imux_inf){
//        fprintf(Echo,"name=%s, pbtype[0]=%s, pbtype[1]=%s, pbtype[2]=%s, arch_imux_switch=%d, lut_group=%d\n", imi.name.c_str(), imi.pbtype_names[0].c_str(), imi.pbtype_names[1].c_str(), imi.pbtype_names[2].c_str(), imi.arch_imux_switch, imi.lut_group);
//        for(auto const& igf : imi.imux_group_inf){
//            for(auto const& ln : igf.lut_name){
//                fprintf(Echo,"\tlut_name=%s", ln.c_str());
//            }
//            fprintf(Echo,"\tmux_num_foreach=%d\n", igf.mux_nums_eachlut);
//
//            for(auto const& fi : igf.imux_from_inf){
//                fprintf(Echo,"\t\tfrom_name.size=%d, from_name=%s, from_start=%d, total_froms=%d, num_foreach=%d, offset=%d, from_group=%d, to_group=%d, pos_order=%d, overlap=%d\n", fi.from_name.size(), fi.from_name[0].c_str(), fi.from_start, fi.total_froms, fi.num_foreach, fi.offset, fi.from_group, fi.to_group, fi.pos_order, fi._overlap);
//                fprintf(Echo,"\t\t\tfrom_switchpoint_nums=%d, pin_type_nums=%d\n",fi.from_switchpoints.size(),fi.pin_types.size());
//                fprintf(Echo,"\t\t\tonly_detail=%d, detail_inf_nums=%d\n", fi._only_detail, fi._imux_detail_inf.size());
//                if(fi._to_fix_order.size()>0){
//                    fprintf(Echo,"\t\t\tto_fix_order[0]=%d,to_fix_order[last]=%d\n",fi._to_fix_order[0],fi._to_fix_order[fi._to_fix_order.size()-1]);
//                }
//            }
//
//        }
//    }
    ...
    fprintf(Echo,"gsb information:");
    fprintf(Echo,"gsb_inf.size=%d\n", arch->gsb_inf.size());
    for(auto const& gf : arch->gsb_inf){
        fprintf(Echo,"name=%s, pbtype[0]=%s, pbtype[1]=%s, pbtype[2]=%s, gsb_seg_group=%d\n", gf.name.c_str(), gf.pbtype_names[0].c_str(), gf.pbtype_names[1].c_str(), gf.pbtype_names[2].c_str(), gf.gsb_seg_group);
        for(auto const& ggf : gf.gsb_group_inf){
            for(auto const& sn : ggf.seg_name){
                fprintf(Echo,"\tseg_name=%s", sn.c_str());
            }
            fprintf(Echo,"\ttrack_nums=%d\n", ggf.track_nums);

            for(auto const& fi : ggf.gsb_from_inf) {
                fprintf(Echo,"\t\tfrom_name.size=%d, from_name=%s, from_start=%d, total_froms=%d, num_foreach=%d, offset=%d, from_group=%d, to_group=%d, pos_order=%d\n", fi.from_name.size(), fi.from_name[0].c_str(), fi.from_start, fi.total_froms, fi.num_foreach, fi.offset, fi.from_group, fi.to_group, fi.pos_order);
                fprintf(Echo,"\t\t\tfrom_switchpoint_nums=%d, pin_type_nums=%d\n",fi.from_switchpoints.size(),fi.pin_types.size());
                fprintf(Echo,"\t\t\tonly_detail=%d, detail_inf_nums=%d\n", fi._only_detail, fi._gsb_detail_inf.size());
                fprintf(Echo,"\t\t\tonly_use_group=%d, todir_group_inf_nums=%d\n", fi._only_use_group, fi._gsb_todir_group_inf.size());
            }
        }

        fprintf(Echo, "gsb_multistage_mux_inf\n");
        for(auto const& gmf : gf.gsb_multistage_mux_inf){
            for (auto const& sn : gmf.to_seg_names) {
                fprintf(Echo, "\tto_seg_name=%s\n", sn.c_str());
            }
            for (auto const& sn : gmf.to_tracks) {
                fprintf(Echo, "\tto_track=%s%d\n", SIDE_STRING[sn.first], sn.second);
            }

            fprintf(Echo, "\treuse_mux\n");
            for(auto const& smi : gmf.reuse_mux_inf){
                fprintf(Echo, "\t\tfrom_name=%s, num_from_details=%d\n", smi.from_name.c_str(), smi.from_inds.size());
            }
            fprintf(Echo, "\tnormal_mux\n");
            for(auto const& nmi : gmf.normal_mux_infs) {
                for(auto const& smi : nmi){
                    fprintf(Echo, "\t\tfrom_name=%s, num_from_details=%d\n", smi.from_name.c_str(), smi.from_inds.size());
                }
            }
        }
