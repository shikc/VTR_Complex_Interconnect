    ...
    for (size_t inode = 0; inode < device_ctx.rr_nodes.size(); inode++) {
        ...
        for (int iedge = 0; iedge < num_edges; iedge++) {
            ...
            for (auto kv : switch_counts) {
                ...
                int to_xlow, to_ylow, to_xhigh, to_yhigh, to_ptc_num, to_capacity;
                t_rr_type to_rr_type;
                t_type_ptr to_type;
                int to_cost_index;

                auto switch_type = device_ctx.rr_switch_inf[kv.first].type();
                auto switch_name = device_ctx.rr_switch_inf[kv.first].name;
                ...
                to_capacity = device_ctx.rr_nodes[to_node].capacity();
                to_cost_index = device_ctx.rr_nodes[to_node].cost_index();
                VPR_THROW(VPR_ERROR_ROUTE, "in check_rr_graph: node %d has %d redundant connections to node %d of switch name %s (%s) from_xlow=%d from_xhigh=%d from_ylow=%d, from_yhigh=%d from_type=%s from_ptc_num=%d, ** to_xlow=%d to_xhigh=%d to_ylow=%d, to_yhigh=%d to_type=%s to_ptc_num=%d",
                          inode, kv.second, to_node, switch_name, SWITCH_TYPE_STRINGS[size_t(switch_type)], xlow, xhigh, ylow, yhigh, rr_node_typename[rr_type], ptc_num, to_xlow, to_xhigh, to_ylow, to_yhigh, rr_node_typename[to_rr_type], to_ptc_num);
            }
        }
...
void check_rr_node(int inode, enum e_route_type route_type, const DeviceContext& device_ctx) {
    ...
    if (xlow < 0 || xhigh > int(grid.width()) - 1 || ylow < 0 || yhigh > int(grid.height()) - 1) {
        vpr_throw(VPR_ERROR_ROUTE, __FILE__, __LINE__,
                  "in check_rr_node: rr endpoints (%d,%d) and (%d,%d) are out of range. inode = %d. rr_type %d. ptc_num = %d\n", xlow, ylow, xhigh, yhigh, inode, rr_type, ptc_num);
    }
    ...
    switch (rr_type) {
        ...
        case SINK:
            ...
            if (xlow != (xhigh - type->width + 1) || ylow != (yhigh - type->height + 1)) {
                if(xlow == xhigh && ylow == yhigh && (xlow == OPEN || ylow == OPEN)) {
                    vpr_throw(VPR_ERROR_ROUTE, __FILE__, __LINE__,
