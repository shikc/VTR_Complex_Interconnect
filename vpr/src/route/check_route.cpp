...
static void check_source(int inode, ClusterNetId net_id) {
    ...
    if(i < blk_x || i > blk_x + type->width - 1 || j < blk_y || j > blk_y + type->height - 1){
        vpr_throw(VPR_ERROR_ROUTE, __FILE__, __LINE__,
...
static bool check_adjacent(int from_node, int to_node) {
    ...
    if((from_type == CHANX || from_type == CHANY) && (to_type == CHANX || to_type == CHANY)) {
        int from_cost_index = device_ctx.rr_nodes[from_node].cost_index();
        int to_cost_index = device_ctx.rr_nodes[to_node].cost_index();
        int from_seg_index = device_ctx.rr_indexed_data[from_cost_index].seg_index;
        int to_seg_index = device_ctx.rr_indexed_data[to_cost_index].seg_index;
        std::string from_seg_name = device_ctx.arch->Segments[from_seg_index].name;
        std::string to_seg_name = device_ctx.arch->Segments[to_seg_index].name;
        if (from_seg_name == "omux_medium" && to_seg_name == "imux_medium")
            num_adj = num_adj == 0 ? 1 : num_adj;
    }
    if (num_adj == 1)
        return (true);
    else if (num_adj == 0)
