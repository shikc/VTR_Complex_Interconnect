    ...
    for (const t_trace* trace = head; trace != nullptr; trace = trace->next) {
        ...
        if (prev_switch != OPEN) {
            ...
            if (!route_tree_connections.count(conn)) {
                //print error conn node inf
                auto& device_ctx = g_vpr_ctx.device();
                t_rr_type rr_type = device_ctx.rr_nodes[prev_node].type();
                int ilow = device_ctx.rr_nodes[prev_node].xlow();
                int jlow = device_ctx.rr_nodes[prev_node].ylow();
                VTR_LOG("Node:\t%d\t%6s (%d,%d) ", prev_node,device_ctx.rr_nodes[prev_node].type_string(), ilow, jlow);
                if ((ilow != device_ctx.rr_nodes[prev_node].xhigh()) || (jlow != device_ctx.rr_nodes[prev_node].yhigh()))
                    VTR_LOG("to (%d,%d) ", device_ctx.rr_nodes[prev_node].xhigh(),device_ctx.rr_nodes[prev_node].yhigh());
                switch (rr_type) {
                    case IPIN:
                    case OPIN:
                    ...
                    default:
                        vpr_throw(VPR_ERROR_ROUTE, __FILE__, __LINE__,
                                  ...
                                  rr_type, device_ctx.rr_nodes[prev_node].type_string());
                        break;
                }
                VTR_LOG("%d  ", device_ctx.rr_nodes[prev_node].ptc_num());


                rr_type = device_ctx.rr_nodes[to_node].type();
                ilow = device_ctx.rr_nodes[to_node].xlow();
                jlow = device_ctx.rr_nodes[to_node].ylow();
                VTR_LOG("Node:\t%d\t%6s (%d,%d) ", to_node,device_ctx.rr_nodes[to_node].type_string(), ilow, jlow);
                if ((ilow != device_ctx.rr_nodes[to_node].xhigh()) || (jlow != device_ctx.rr_nodes[to_node].yhigh()))
                    VTR_LOG("to (%d,%d) ", device_ctx.rr_nodes[to_node].xhigh(),device_ctx.rr_nodes[to_node].yhigh());
                switch (rr_type) {
                    case IPIN:
                    case OPIN:
                        if (is_io_type(device_ctx.grid[ilow][jlow].type)) {
                    ...
                    default:
                        vpr_throw(VPR_ERROR_ROUTE, __FILE__, __LINE__,
                                  "in print_route: Unexpected traceback element type: %d (%s).\n",
                                  rr_type, device_ctx.rr_nodes[to_node].type_string());
                        break;
                }
                VTR_LOG("%d  ", device_ctx.rr_nodes[to_node].ptc_num());
