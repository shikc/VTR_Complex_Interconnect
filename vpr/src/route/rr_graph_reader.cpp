...
void load_rr_file(const t_graph_type graph_type,
    ...
    try {
        ...
        alloc_and_load_rr_indexed_data(segment_inf, device_ctx.medium_seg_inf, device_ctx.rr_node_indices, device_ctx.medium_rr_node_indices,
...
void set_cost_indices(pugi::xml_node parent, const pugiutil::loc_data& loc_data, const bool is_global_graph, const int num_seg_types) {
    ...
    for (size_t i = 0; i < device_ctx.rr_nodes.size(); i++) {
        ...
        segmentSubnode = get_single_child(rr_node, "segment", loc_data, OPTIONAL);
        if (segmentSubnode) {
            attribute = get_attribute(segmentSubnode, "cost_index", loc_data, OPTIONAL);
            if (attribute) {
                int cost_id = get_attribute(segmentSubnode, "cost_index", loc_data).as_int(0);
                if (is_global_graph) {
                    node.set_cost_index(0);
                } else if (device_ctx.rr_nodes[i].type() == CHANX or device_ctx.rr_nodes[i].type() == CHANY) {
                    node.set_cost_index(cost_id);
                }
            }
        }
        rr_node = rr_node.next_sibling(rr_node.name());
