...
struct t_pt_to_pt_detailICs {
    t_rr_type from_rr_type = NUM_RR_TYPES;
    const t_type_descriptor* from_clb_type;
    int from_pt_start_index;
    int from_pt_end_index;
    t_rr_type to_rr_type = NUM_RR_TYPES;
    const t_type_descriptor* to_clb_type;
    int to_pt_start_index;
    int to_pt_end_index;
    int switch_index; //The switch type used by this detail connection
};
...
static void build_unidir_rr_opins(const int i,
                                  ...
                                  const e_side side,
                                  const DeviceGrid& grid,
                                  const std::vector<vtr::Matrix1<int>>& Fc_out,
                                  const int max_chan_width,
                                  const t_chan_details& chan_details_x,
                                  const t_chan_details& chan_details_y,
                                  vtr::NdMatrix<int, 4>& Fc_xofs,
                                  vtr::NdMatrix<int, 4>& Fc_yofs,
                                  t_rr_edge_info_set& created_rr_edges,
                                  bool* Fc_clipped,
                                  const t_rr_node_indices& L_rr_node_indices,
                                  const std::vector<t_rr_node>& rr_nodes,
...
static void build_unidir_rr_opins_ipins(const int i,
                                  const int j,
                                  const e_side side,
                                  const DeviceGrid& grid,
                                  vtr::NdMatrix<std::vector<int>, 3> block_ipin,
...
static int get_detailICs_connections(t_rr_edge_info_set& rr_edges_to_create,
                                    const t_rr_node_indices& L_rr_node_indices,
                                    const std::vector<t_rr_node>& rr_nodes,
                                    const t_detailInterConnect_inf* detailICs,
                                    const int num_detailICs,
                                    const t_pt_to_pt_detailICs* pt_to_pt_detailICs);

static void alloc_and_load_rr_graph_gsb(const int num_nodes,
                                        ...
                                        t_two_stage_mux_map* gsb_two_stage_mux_map,
...
//const std::vector<int> gsb_medium_id_vec
...
static std::vector<std::vector<bool>> alloc_and_load_perturb_ipins(const int L_num_types,
                                                                   const int num_seg_types,
                                                                   const int* sets_per_seg_type,
                                                                   const std::vector<vtr::Matrix1<int>>& Fc_in,
                                                                   const std::vector<vtr::Matrix1<int>>& Fc_out,
                                                                   const enum e_directionality directionality);
...
static int build_rr_chan_to_ipin(const int x_coord,
                                    const int y_coord,
...
static int build_rr_chan_accord_sb_when_gsb(const int x_coord,
                                            const int y_coord,
                                            const t_rr_type chan_type,
                                            const int max_chan_width,
                                            const DeviceGrid& grid,
...
static void build_bend_rr_chan(const int x_coord,
                               const int y_coord,
                               const t_rr_type chan_type,
                               const int cost_index_offset,
                               const int max_chan_width,
...
static void init_rr_node_gsb_arch(const int x_coord,
                                  const int y_coord,
                                  const t_rr_type chan_type,
                                  const int cost_index_offset,
                                  const int tracks_per_chan,
...
static int build_gsb_arch(const int x_coord,
                          const int y_coord,
                          const std::map<std::string, std::pair<int, int>>& record_imux_coord, //for imux
                          const DeviceGrid& grid,
                          const t_chan_details& chan_details_x,
                          const t_chan_details& chan_details_y,
                          const t_rr_node_indices& L_rr_node_indices,
...
static t_pt_to_pt_detailICs* alloc_and_load_pt_to_pt_detailICs(const t_detailInterConnect_inf* detailICs,
                                                                 const int num_detailICs, 
                                                                 const int delayless_switch, 
                                                                 const std::vector<t_segment_inf>& segment_inf,
                                                                 const std::vector<int> nums_per_segtype,
...
static std::vector<vtr::Matrix1<int>> alloc_and_load_actual_fc(const int L_num_types,
...
void create_rr_graph(const t_graph_type graph_type,
    ...
    if (channel_widths_unchanged(device_ctx.normal_chan_width, nodes_per_chan) && !device_ctx.rr_nodes.empty()) {
...
static void build_rr_graph(const t_graph_type graph_type,
    ...
    *Warnings = RR_GRAPH_NO_WARN;
    ...
    //check whether the channei width is right
    auto const & seg_groups = arch->gsb_inf[0].gsb_group_inf;
    int chan_w = 0;
    for(auto it = seg_groups.begin(); it != seg_groups.end(); it++){
        for(auto it2 = segment_inf.begin(); it2 != segment_inf.end(); it2++){
            if((*it2).name == (*it).seg_name[0]){
                chan_w += (*it2).length * (*it).track_nums * 2;
                break;
            }
        }
    }
    if(chan_w != max_chan_width){
        vpr_throw(VPR_ERROR_OTHER, __FILE__, __LINE__,
                  "Chan_width is wrong! The right chan_width is %d!", chan_w);
    }
    ...
    std::vector<int> nums_per_segtype(segment_inf.size());
    if (is_global_graph) {
        ...
        seg_details = alloc_and_load_seg_details(&max_chan_width,
                                                 max_dim, arch, medium_seg_inf,
                                                 segment_inf, use_full_seg_groups, is_global_graph,
                                                 directionality, nums_per_segtype, clbpin_medium_map, 
                                                 mux_name_medium_map_vec, use_bend_segment_groups, &num_seg_details);
    ...
    alloc_and_load_chan_details(grid, &nodes_per_chan,
                                ...
                                num_seg_details, seg_details, medium_seg_inf,
    ...
    /* START PT_TO_PT */
    t_pt_to_pt_detailICs* pt_to_pt_detailICs = nullptr;
    

    /* START FC */
    /* Determine the actual value of Fc */  //addered by shikc in 200103
    std::vector<vtr::Matrix1<int>> Fc_in;  /* [0..device_ctx.num_block_types-1][0..num_pins-1][0..3][0..num_segments-1] */
    std::vector<vtr::Matrix1<int>> Fc_out; /* [0..device_ctx.num_block_types-1][0..num_pins-1][0..3][0..num_segments-1] */
    ...
    if (is_global_graph) {
        ...
        auto ones = vtr::Matrix1<int>({size_t(max_pins), 4, segment_inf.size()}, 1);
        Fc_in = std::vector<vtr::Matrix1<int>>(L_num_types, ones);
        Fc_out = std::vector<vtr::Matrix1<int>>(L_num_types, ones);
    } else {
				...
				for (int m = 0; m < 4; ++m) {
                	for (size_t k = 0; k < segment_inf.size(); k++) {
                    	...
                    	VTR_LOG(
                        	...
                        	Fc_in[i][j][k]);
#endif /* VERBOSE */
                    	VTR_ASSERT_MSG(Fc_out[i][j][m][k] == 0 || Fc_in[i][j][m][k] == 0,
    ...
    //[0..grid_width-1][0..grid_height-1][IMUX/OMUX/GSB][0..num_omuxs/imuxs_types/gsb_types-1][0..pin_num-1]
    device_ctx.medium_rr_node_indices = alloc_and_load_medium_rr_node_indices(grid, &num_rr_nodes, arch, medium_seg_inf, chan_details_x, chan_details_y);
     ...
     * wire start points */
    vtr::NdMatrix<int, 4> Fc_xofs({grid.height() - 1,
                                   grid.width() - 1,
                                   segment_inf.size(),
                                   2},
                                  0); //[0..grid.height()-2][0..grid.width()-2][0..num_seg_types-1]
    vtr::NdMatrix<int, 4> Fc_yofs({grid.width() - 1,
                                   grid.height() - 1,
                                   segment_inf.size(),
                                   2},
        ...
        if (sb_type == CUSTOM) {
            ...
            for (size_t i = 0; i < grid.width() - 1; i++) {
                for (size_t j = 0; j < grid.height() - 1; j++) {
                    ...
                    auto pb_type_name = grid[i][j].type->name;
                    for(auto const& gsb : arch->gsb_inf){
                        for(auto gsb_pbtype_name : gsb.pbtype_names){
                            if( 0 == strcmp(pb_type_name, gsb_pbtype_name.c_str())){
                                is_gsb_arch = true;
                                break;
                            }
                        }
                        if (is_gsb_arch) break;
                    }
                    if(!is_gsb_arch){
                        ...
                        load_sblock_bend_pattern_lookup_for_gsb(i, j, grid, &nodes_per_chan,
                                                                chan_details_x, chan_details_y,
                                                                Fs, sb_type, unidir_sb_pattern);
                    }
                }
    ...
    std::map<std::string, std::vector<std::map<int, int>>> other_pbpin_to_clbpin_map;
    build_other_pbpin_to_clbpin(arch->gsb_inf[0], other_pbpin_to_clbpin_map); 

    
    t_gsb_connection_map* gsb_conn_map = nullptr;
    t_gsb_multistage_mux_map* gsb_multistage_mux_map = new t_gsb_multistage_mux_map;
    t_two_stage_mux_map* gsb_stage_mux_map = nullptr;
    if(arch->gsb_inf[0].twostage_mux_spec){
        gsb_stage_mux_map = alloc_and_load_two_stage_map(chan_details_x, chan_details_y, grid, arch->gsb_inf,
                                                              segment_inf, &nodes_per_chan, other_pbpin_to_clbpin_map,
                                                              mux_name_medium_map_vec[0], clbpin_medium_map, wire_to_arch_ipin_switch);
    }else{
        gsb_conn_map = alloc_and_load_gsb_permutations(chan_details_x, chan_details_y, grid, arch->gsb_inf, &nodes_per_chan, gsb_multistage_mux_map, other_pbpin_to_clbpin_map);
    }
    ...
    bool Fc_clipped = false;
    std::map<int, int> bend_segment_map;
    alloc_and_load_rr_graph_gsb(device_ctx.rr_nodes.size(), device_ctx.rr_nodes, segment_inf.size(),
                                chan_details_x, chan_details_y, track_to_pin_lookup,
                                grid, Fs, unidir_sb_pattern,
                                Fc_out, Fc_xofs, Fc_yofs,
                                device_ctx.rr_node_indices, device_ctx.medium_rr_node_indices,
                                max_chan_width,
                                nodes_per_chan,
                                ...
                                detailICs, num_detailics, pt_to_pt_detailICs,
                                is_global_graph, arch,
                                clbpin_medium_map, medium_seg_inf,
                                gsb_conn_map,
                                gsb_multistage_mux_map,
                                gsb_stage_mux_map,
                                other_pbpin_to_clbpin_map, 
                                use_bend_segment_groups,
                                bend_segment_map, segment_inf);
    ...
    device_ctx.medium_seg_inf = medium_seg_inf;
    device_ctx.normal_chan_width = nodes_per_chan;
    int medium_num = medium_seg_inf.gsb_mdeium_num_xory;
    for(size_t x = 0; x < grid.width(); x++){
        nodes_per_chan.y_list[x] += medium_num;
    }
    for(size_t y = 0; y < grid.height(); y++){
        nodes_per_chan.x_list[y] += medium_num;
    }
    nodes_per_chan.max += medium_num;
    nodes_per_chan.x_max += medium_num;
    nodes_per_chan.y_max += medium_num;
    //Save the channel widths for the newly constructed graph
    device_ctx.chan_width = nodes_per_chan;

    rr_graph_externals(segment_inf, medium_seg_inf, max_chan_width,
    ...
    if (gsb_conn_map) {
        free_permutations(gsb_conn_map);
        gsb_conn_map = nullptr;
    }
    if(gsb_multistage_mux_map){
        free_permutations(gsb_multistage_mux_map);
        gsb_multistage_mux_map = nullptr;
    }

    if(gsb_stage_mux_map){
        free_permutations(gsb_stage_mux_map);
        gsb_stage_mux_map = nullptr;
    }
    ...
    if (pt_to_pt_detailICs != nullptr) {
        free(pt_to_pt_detailICs);
    }
}
...
static void alloc_rr_switch_inf(t_arch_switch_fanin& arch_switch_fanins) {
    ...
    int num_rr_switches = 0;
    {
        ...
        for (size_t inode = 0; inode < device_ctx.rr_nodes.size(); ++inode) {
            if(inward_switch_inf[inode].size() == 2){
                auto& temp = inward_switch_inf[inode];
                for(auto it=temp.begin();it!=temp.end();it++){
                    string name = device_ctx.arch_switch_inf[it->first].name;
                    if(name == "__vpr_delayless_switch__") {
                        int num = it->second;
                        temp.erase(it);
                        temp.begin()->second += num;
                        break;
                    }
                }
        ...
        /*
        for(auto it=inward_switch_inf[6745].begin();it!=inward_switch_inf[6745].end();it++){
            VTR_LOG("%d:%d\n", it->first,it->second);
        }
        auto name1 = device_ctx.arch_switch_inf[15].name;
        auto name2 = device_ctx.arch_switch_inf[16].name;
        */
...
static void remap_rr_node_switch_indices(const t_arch_switch_fanin& switch_fanin) {
    ...
    for (size_t inode = 0; inode < device_ctx.rr_nodes.size(); inode++) {
        ...
        for (int iedge = 0; iedge < num_edges; iedge++) {
            int ito_node = from_node.edge_sink_node(iedge);
            ...
            /*if(itr == switch_fanin[switch_index].end()){
                for(auto ii=0;ii<switch_fanin.size();ii++){
                    auto sw_map = switch_fanin[ii];
                    VTR_LOG("switch_index:%d\n",ii);
                    for(auto it2=sw_map.begin();it2!=sw_map.end();it2++){
                        VTR_LOG("fain_in:%d\tindex:%d\n",it2->first, it2->second);
                    }
                }
                int a = 0;
                a++;
...
static void rr_graph_externals(const std::vector<t_segment_inf>& segment_inf,
    ...
    alloc_and_load_rr_indexed_data(segment_inf, medium_seg_inf, device_ctx.rr_node_indices,
                                   device_ctx.medium_rr_node_indices, max_chan_width,
                                   wire_to_rr_ipin_switch, base_cost_type);
    load_rr_index_segments(segment_inf.size(), segment_inf);
}
//addered by shikc in 200103
static std::vector<std::vector<bool>> alloc_and_load_perturb_ipins(const int L_num_types,
                                                                   const int num_seg_types,
                                                                   const int* sets_per_seg_type,
                                                                   const std::vector<vtr::Matrix1<int>>& Fc_in,
    ...
    if (BI_DIRECTIONAL == directionality) {
        for (int iseg = 0; iseg < num_seg_types; ++iseg) {
            ...
            for (int itype = 1; itype < L_num_types; ++itype) {
                ...
                float Fc_ratio;
                if (Fc_in[itype][0][0][iseg] > Fc_out[itype][0][0][iseg]) {
                    Fc_ratio = (float)Fc_in[itype][0][0][iseg] / (float)Fc_out[itype][0][0][iseg];
                } else {
                    Fc_ratio = (float)Fc_out[itype][0][0][iseg] / (float)Fc_in[itype][0][0][iseg];
                }

                if ((Fc_in[0][itype][0][iseg] <= tracks_in_seg_type - 2)
...
static std::vector<vtr::Matrix1<int>> alloc_and_load_actual_fc(const int L_num_types,
    ...
    auto zeros = vtr::Matrix1<int>({size_t(max_pins), 4, segment_inf.size()}, 0);
    std::vector<vtr::Matrix1<int>> Fc(L_num_types, zeros);
            ...
            for(int i=0; i<4; i++) {
                if (fc_spec.fc_value[i] == 0) {
                    ...
                    if (fc_spec.fc_value_type == e_fc_value_type::FRACTIONAL) {
                        float conns_per_pin = fac * sets_per_seg_type[iseg] * fc_spec.fc_value[i];
                    	...
                    	total_connections = vtr::nint(fc_spec.fc_value[i]) * fc_spec.pins.size();
                	}
                    ...
                    for (int ipin : fc_spec.pins) {
                        ...
                        if (Fc[itype][ipin][i][iseg] > sets_per_seg_type[iseg] * fac) {
                            *Fc_clipped = true;
                            Fc[itype][ipin][i][iseg] = sets_per_seg_type[iseg] * fac;
                        }

                        //I encountered a situation which has a small Fc and need to ensure Fc[itype][ipin][iseg] != 0
                        //othersise some problems arise  ---- by Sun Xibo
                        //if (Fc[itype][ipin][iseg] == 0)
                        //    Fc[itype][ipin][iseg] = fac ;

                        VTR_ASSERT_MSG(Fc[itype][ipin][i][iseg] >= 0, "Calculated absolute Fc must be positive");
                        VTR_ASSERT_MSG(Fc[itype][ipin][i][iseg] % fac == 0, "Calculated absolute Fc must be divisible by 1 (bidir architecture) or 2 (unidir architecture)"); //Required by connection block construction code
                    }
                }
            }
        }
                    ...
                    for (int height = 0; height < types[i].height; ++height) {
                        for (int side = 0; side < 4; ++side) {
                            for (int idx = 0; idx < 4; ++idx) {
                                if (!track_to_pin_map[i][track][width][height][side][idx].empty()) {
                                    track_to_pin_map[i][track][width][height][side][idx].clear();
                                }
                            }
                        }
                    }
...
/* gsb version: alloc_and_load_rr_graph*****************************
...
static void alloc_and_load_rr_graph_gsb(const int num_nodes,
    ...
    *Fc_clipped = false;
            ...
            for (e_side side : SIDES) {
                if (BI_DIRECTIONAL == directionality) {
                    ...
                    for (int pin_index = 0; pin_index < type->num_pins; ++pin_index) {
                        ...
                        get_opin_direct_connecions(i, j, side, pin_index, opin_node_index, rr_edges_to_create, L_rr_node_indices, L_rr_node,
                                                   directs, num_directs, clb_to_clb_directs);
                    }
                }
    ...
    /* load the num_segment, cut off the bend segment */
    ...
    for (size_t i = 0; i < grid.width() - 1; ++i) {
        for (size_t j = 0; j < grid.height() - 1; ++j) {
            if (i > 0) {
                ...
                init_rr_node_gsb_arch(i, j, CHANX, CHANX_COST_INDEX_START,
                                      tracks_per_chan, medium_seg_inf, chan_details_x, chan_details_y,
                                      L_rr_node_indices, L_medium_rr_node_indices, L_rr_node);
            }
            if (j > 0) {
                int tracks_per_chan = ((is_global_graph) ? 1 : chan_width.y_list[i]);
                init_rr_node_gsb_arch(i, j, CHANY, CHANX_COST_INDEX_START + real_num_segment,
                                      tracks_per_chan, medium_seg_inf, chan_details_x, chan_details_y,
                                      L_rr_node_indices, L_medium_rr_node_indices, L_rr_node);
            }
        }
    ...
    for (size_t i = 0; i < grid.width(); ++i) {
        for (size_t j = 0; j < grid.height(); ++j) {
            ...
            for (e_side side : SIDES) {
                ...
                filter_midpoint_edges(rr_edges_to_create, segment_inf);
                uniquify_edges(rr_edges_to_create);
                alloc_and_load_edges(L_rr_node, rr_edges_to_create);
                rr_edges_to_create.clear();
            }
    ...
    for (size_t i = 0; i < grid.width() - 1; ++i) {
        for (size_t j = 0; j < grid.height() - 1; ++j) {
            if((i == 0 || j == 0 || i == grid.width() - 1 || j == grid.height()-1) && grid[i][j].type == device_ctx.EMPTY_TYPE){
                continue;
            }
            int total_edges_sb = 0;
            int side_edges_ipin = 0;
            bool is_gsb_arch = false;
            auto pb_type_name = grid[i][j].type->name;
            for(auto const& gsb : arch->gsb_inf){
                for (auto gsb_pbtype_name : gsb.pbtype_names) {
            ...
            if(is_gsb_arch){
                auto right_pb_type_name = grid[i+1][j].type->name;
                auto top_pb_type_name = grid[i][j+1].type->name;
                bool right_not_gsb_arch = true;
                bool top_not_gsb_arch = true;
                for (auto gsb_pbtype_name : arch->gsb_inf[0].pbtype_names) {
                    if (0 == strcmp(right_pb_type_name, gsb_pbtype_name.c_str())) 
                        right_not_gsb_arch = false;
                    if (0 == strcmp(top_pb_type_name, gsb_pbtype_name.c_str())) 
                        top_not_gsb_arch = false;
                }

                if(i + 1 == grid.width() - 1 && grid[i+1][j].type == device_ctx.EMPTY_TYPE){
                    right_not_gsb_arch = false;
                }
                if(j + 1 == grid.height() - 1 && grid[i][j+1].type == device_ctx.EMPTY_TYPE){
                    top_not_gsb_arch = false;
                }

                if( right_not_gsb_arch ) {
                    int tracks_per_chan = ((is_global_graph) ? 1 : chan_width.y_list[i]);
                    side_edges_ipin = build_rr_chan_to_ipin(i, j, CHANY, track_to_pin_lookup,
                                          tracks_per_chan, chan_details_x, chan_details_y,
                                          L_rr_node_indices, rr_edges_to_create,
                                          wire_to_ipin_switch,
                                          directionality);
                ...
                if( top_not_gsb_arch ) {
                    int tracks_per_chan = ((is_global_graph) ? 1 : chan_width.x_list[j]);
                    side_edges_ipin = build_rr_chan_to_ipin(i, j, CHANX, track_to_pin_lookup,
                                          tracks_per_chan, chan_details_x, chan_details_y,
                                          L_rr_node_indices, rr_edges_to_create,
                                          wire_to_ipin_switch,
                                          directionality);
                ...
                if (i > 0) {
                    ...
                    total_edges_sb += build_rr_chan_accord_sb_when_gsb(i, j, CHANX,
                                                        max_chan_width, grid, tracks_per_chan,
                                                        sblock_pattern, Fs / 3, chan_details_x, chan_details_y,
                                                        L_rr_node_indices, rr_edges_to_create, bend_delayless_switch,
                                                        directionality, use_short_segment_groups, bend_segment_map, INC_DIRECTION);
                ...
                if (i + 1 < grid.width() - 1) {
                    int tracks_per_chan = ((is_global_graph) ? 1 : chan_width.x_list[j]);
                    total_edges_sb += build_rr_chan_accord_sb_when_gsb(i + 1, j, CHANX,
                                                        max_chan_width, grid, tracks_per_chan,
                                                        sblock_pattern, Fs / 3, chan_details_x, chan_details_y,
                                                        L_rr_node_indices, rr_edges_to_create, bend_delayless_switch,
                                                        directionality, use_short_segment_groups, bend_segment_map, DEC_DIRECTION);
                ...
                if (j > 0) {
                    ...
                    side_edges_ipin = build_rr_chan_to_ipin(i, j, CHANY, track_to_pin_lookup,
                                          ...
                                          wire_to_ipin_switch,
                                          directionality);
                    total_edges_sb += build_rr_chan_accord_sb_when_gsb(i, j, CHANY,
                                                        max_chan_width, grid, tracks_per_chan,
                                                        sblock_pattern, Fs / 3, chan_details_x, chan_details_y,
                                                        L_rr_node_indices, rr_edges_to_create, bend_delayless_switch,
                                                        directionality, use_short_segment_groups, bend_segment_map, INC_DIRECTION);
                ...
                if (j + 1 < grid.height() - 1) {
                    int tracks_per_chan = ((is_global_graph) ? 1 : chan_width.y_list[i]);
                    total_edges_sb += build_rr_chan_accord_sb_when_gsb(i, j + 1, CHANY,
                                                        max_chan_width, grid, tracks_per_chan,
                                                        sblock_pattern, Fs / 3, chan_details_x, chan_details_y,
                                                        L_rr_node_indices, rr_edges_to_create, bend_delayless_switch,
                                                        directionality, use_short_segment_groups, bend_segment_map, DEC_DIRECTION);
    ...
    std::map<std::string, std::pair<int, int>> record_imx_coord;
    for (size_t i = 0; i < grid.width() - 1; ++i) {
        for (size_t j = 0; j < grid.height() - 1; ++j) {
            t_type_ptr type = grid[i][j].type;
            for(auto imx : arch->gsb_inf){
                if (0 != strcmp(type->name, imx.pbtype_names[0].c_str())) //因为以第一种为标准，因此记录的是第一次发现的第一种pb的位置，而非后面的pb位置
                    continue;

                std::pair<int, int> p = make_pair(i, j);
                auto it = record_imx_coord.find(imx.pbtype_names[0]);
                if(it == record_imx_coord.end()){
                    record_imx_coord[imx.pbtype_names[0]] = p;
                }
                break;
            }
            if(record_imx_coord.size() == arch->gsb_inf.size()) break;
        }
        if(record_imx_coord.size() == arch->gsb_inf.size()) break;
    }
    ...
    for (size_t i = 0; i < grid.width() - 1; ++i) {
        for (size_t j = 0; j < grid.height() - 1; ++j) {
            int edge_count = build_gsb_arch(i, j, record_imx_coord, grid, chan_details_x, chan_details_y,
                                            L_rr_node_indices, L_medium_rr_node_indices,
                                            rr_edges_to_create, clbpin_to_medium,
                                            gsb_conn_map,
                                            gsb_two_stage_mux_map,
                                            other_pbpin_to_clbpin_map, arch, gsb_multistage_mux_map, delayless_switch);
            //Create the actual CHAN->CHAN edges
            uniquify_edges(rr_edges_to_create);
            alloc_and_load_edges(L_rr_node, rr_edges_to_create);
            rr_edges_to_create.clear();
    ...
    count_wire_type_sizes(chan_details_x[0][0].data(), max_chan_width, &wire_type_sizes);
    connect_bounds_wires(wire_type_sizes, grid, chan_details_x, chan_details_y, L_rr_node_indices, rr_edges_to_create, delayless_switch);
    uniquify_edges(rr_edges_to_create);
    alloc_and_load_edges(L_rr_node, rr_edges_to_create);
    rr_edges_to_create.clear();

    int atcual_num = get_detailICs_connections(rr_edges_to_create, L_rr_node_indices, L_rr_node,
                                   detailICs, num_detailICs, pt_to_pt_detailICs);
...
static void alloc_and_load_rr_graph(const int num_nodes,
    ...
    *Fc_clipped = false;
    ...
    auto block_ipin = vtr::NdMatrix<std::vector<int>, 3> ({grid.width(),grid.height(),NUM_SIDES},{0});
    for (size_t i = 0; i < grid.width(); ++i) {
        for (size_t j = 0; j < grid.height(); ++j) {
            for (e_side side : SIDES) {
                t_type_ptr type = grid[i][j].type;
                ...
                for (int pin_index = 0; pin_index < type->num_pins; ++pin_index) {
                    ...
                    int class_index = type->pin_class[pin_index];
                    if (type->class_inf[class_index].type != RECEIVER) {
                        continue;
                    }
                    if (type->is_ignored_pin[pin_index]) {
                        continue;
                    ...
                    int ipin_node_index = get_rr_node_index(L_rr_node_indices, i, j, IPIN, pin_index, side);
                    if (ipin_node_index < 0) continue;
                    if (0 == type->pinloc[width_offset][height_offset][side][pin_index]) {
                        continue;
                    }
                    block_ipin[i][j][side].push_back(ipin_node_index);
                }
            }
        }
    }
    for (size_t i = 1; i < grid.width()-1; ++i) {//显示问题所在
        for (size_t j = 1; j < grid.height()-1; ++j) {
            for (e_side side : SIDES) {
                if (BI_DIRECTIONAL == directionality) {
                    ...
                    VTR_ASSERT(UNI_DIRECTIONAL == directionality);
                    build_unidir_rr_opins_ipins(i, j, side, grid, block_ipin, wire_to_ipin_switch,
                                          rr_edges_to_create,  L_rr_node_indices, L_rr_node
                                          );
                ...
                for (int ipin = 0; ipin < num_pins; ++ipin) {
                    if (type->pinloc[width_offset][height_offset][side][ipin]) {
                        ...
                        if (class_inf[iclass].type == RECEIVER) {
                            ...
                            if(class_inf[iclass].num_pins == 1){
                                L_rr_node[to_node].set_coordinates(i + width_offset, j + height_offset, i + width_offset, j + height_offset);
                            }
                            ...
                            if(class_inf[iclass].num_pins == 1){
                                int source_node = get_rr_node_index(L_rr_node_indices, i, j, SOURCE, iclass);
                                L_rr_node[source_node].set_coordinates(i + width_offset, j + height_offset, i + width_offset, j + height_offset);
                            }
                        }
...
void init_fan_in(std::vector<t_rr_node>& L_rr_node, const int num_rr_nodes) {
    ...
    for (int i = 0; i < num_rr_nodes; i++) {
        for (int iedge = 0; iedge < L_rr_node[i].num_edges(); iedge++) {
            ...
            /*if(L_rr_node[to_node].fan_in() + 1 <0){
                short ii = L_rr_node[to_node].fan_in() + 1;
                ii++;
            }*/
...
// Allocates/loads edges for nodes belonging to specified channel segment(4 side of sb)
static int build_rr_chan_accord_sb_when_gsb(const int x_coord,
                                                const int y_coord,
                                                const t_rr_type chan_type,
                                                const int max_chan_width,
    ...
    for (int track = 0; track < tracks_per_chan; ++track) {
        ...
        if(seg_details[track].direction() != track_direction)
            continue;
        ...
        if (chan_type == CHANY) {
            from_seg_details = chan_details_y[x_coord][y_coord].data();
        } else {
            from_seg_details = chan_details_x[x_coord][y_coord].data();
        }

        int sb_seg;
        if(track_direction == INC_DIRECTION){
            sb_seg = seg_coord ;
        }
        else{
            VTR_ASSERT(track_direction == DEC_DIRECTION);
            sb_seg = seg_coord - 1;
        }
        
        bool from_is_sblock = is_sblock(chan_coord, seg_coord, sb_seg, track,
                                   from_seg_details, directionality);
        if ((track_direction == INC_DIRECTION && seg_coord == end) || (track_direction == DEC_DIRECTION && seg_coord == start)) {
            from_is_sblock = true;
        }
        
        if(!from_is_sblock) continue;
        /*
        if (seg_coord > start)
            continue; // Only process segments which start at this location 
        VTR_ASSERT(seg_coord == start);
        ...
        */
        ...
        if (chan_coord > 0) {
            ...
            if (chan_type == CHANX) {
                to_seg_details = chan_details_y[x_coord][y_coord].data();
            } else {
                VTR_ASSERT(chan_type == CHANY);
                to_seg_details = chan_details_x[x_coord][y_coord].data();
            }
            if (to_seg_details->length() > 0) {
                num_edges += get_track_to_tracks_normal_when_gsb(chan_coord, seg_coord, track, chan_type, chan_coord,
                                                                    opposite_chan_type, sb_seg, seg_dimension, max_chan_width, grid,
                                                                    Fs_per_side, sblock_pattern, node, rr_edges_to_create,
                                                                    from_seg_details, to_seg_details, opposite_chan_details,
                                                                    directionality, L_rr_node_indices, bend_delayless_switch, bend_segment_map);
            }
        }
        if (chan_coord < chan_dimension) {
            const t_chan_seg_details* to_seg_details;
            if (chan_type == CHANX) {
                to_seg_details = chan_details_y[x_coord][y_coord + 1].data();
            } else {
                VTR_ASSERT(chan_type == CHANY);
                to_seg_details = chan_details_x[x_coord + 1][y_coord].data();
            }
            if (to_seg_details->length() > 0) {
                num_edges += get_track_to_tracks_normal_when_gsb(chan_coord, seg_coord, track, chan_type, chan_coord + 1,
                                                                    opposite_chan_type, sb_seg, seg_dimension, max_chan_width, grid,
                                                                    Fs_per_side, sblock_pattern, node, rr_edges_to_create,
                                                                    from_seg_details, to_seg_details, opposite_chan_details,
                                                                    directionality, L_rr_node_indices, bend_delayless_switch, bend_segment_map);
        ...
        int target_seg;
        if(track_direction == INC_DIRECTION && end == seg_coord){
            target_seg = seg_coord + 1;
        }
        else if(track_direction == DEC_DIRECTION && start == seg_coord){
            target_seg = seg_coord - 1;
        }
        else{
            continue;
        ...
        if (target_seg > 0 && target_seg < seg_dimension + 1) {
            ...
            if (to_seg_details->length() > 0) {
                num_edges += get_track_to_tracks_normal_when_gsb(chan_coord, seg_coord, track, chan_type, target_seg,
                                                                    chan_type, sb_seg, seg_dimension, max_chan_width, grid,
                                                                    Fs_per_side, sblock_pattern, node, rr_edges_to_create,
...
static void build_rr_chan(const int x_coord,
    ...
    for (int track = 0; track < tracks_per_chan; ++track) {
        ...
        bool is_bend_end = seg_details[track].bend_len() == seg_details[track].part_idx() + 1;
        ...
        L_rr_node[node].set_bend_end_node(is_bend_end);
...
static void build_bend_rr_chan(const int x_coord,
    ...
    for (int track = 0; track < tracks_per_chan; ++track) {
        ...
        vtr::NdMatrix<std::vector<int>, 3> switch_block_conn;//no use
        bool is_bend_first = (seg_details[track].bend_len() > 0 && seg_details[track].part_idx() == 0);
        int num_edges = 0;
...
/* Allocates/loads edges for nodes belonging to specified channel segment and initializes
 * node properties such as cost, occupancy and capacity */
//init fist and then add edge according gsb/imux/omux map
static void init_rr_node_gsb_arch(const int x_coord,
                                  const int y_coord,
                                  const t_rr_type chan_type,
                                  const int cost_index_offset,
    ...
    for (int track = 0; track < tracks_per_chan; ++track) {
        ...
        /* Edge arrays have now been built up.  Do everything else.  */
        L_rr_node[node].set_seg_id(seg_details[track].index());
        L_rr_node[node].set_cost_index(cost_index_offset + seg_details[track].cost_index());
        L_rr_node[node].set_capacity(1); /* GLOBAL routing handled elsewhere */
        L_rr_node[node].set_bend_first_node(is_bend_first);
        L_rr_node[node].set_bend_end_node(is_bend_end);
    ...
    int inited_track = tracks_per_chan;
    //int actual_gsb_medium_num;
    for(int i = 0; i < 1; i++) {
        int num_medium = medium_seg_inf.gsb_mdeium_num_xory;

        for (int i_m = 0; i_m < num_medium; ++i_m) {
            int track = inited_track + i_m;
            if (seg_details[track].length() == 0)
                continue;
            ...
            int acc_y = chan_type == CHANY ? num_medium : 0;

            int node = get_medium_rr_node_index(L_medium_rr_node_indices, x_coord, y_coord, i, 0, i_m + acc_y); //0-IMUX, 1-OMUX, 2-GSB
            ...
            L_rr_node[node].set_cost_index(cost_index_offset + seg_details[track].cost_index());
            L_rr_node[node].set_capacity(1); /* GLOBAL routing handled elsewhere */
            L_rr_node[node].set_medium_node(true);//set the node is medium_node
...
static int build_gsb_arch(const int x_coord,
                          ...
                          const int delayless_switch) {
    int edge_count = 0;
    std::string actual_pbtype_name = arch->gsb_inf[0].pbtype_names[0];//默认只有一种gsb
    
    if(gsb_conn_map != nullptr){
        //gsb seg
        edge_count += get_track_to_track_for_gsb(x_coord, y_coord, grid, chan_details_x, chan_details_y, L_rr_node_indices, rr_edges_to_create, gsb_conn_map, gsb_multistage_mux_map);

        //gsb omux
        edge_count += get_omuxOrpb_to_track_for_gsb(x_coord, y_coord, grid, chan_details_x, chan_details_y, L_rr_node_indices, L_medium_rr_node_indices,
                                                    rr_edges_to_create, gsb_conn_map, other_pbpin_to_clbpin_map, gsb_multistage_mux_map, e_from_type::FT_OMUX);

        //gsb pb
        edge_count += get_omuxOrpb_to_track_for_gsb(x_coord, y_coord, grid, chan_details_x, chan_details_y, L_rr_node_indices, L_medium_rr_node_indices,
                                                    rr_edges_to_create, gsb_conn_map, other_pbpin_to_clbpin_map, gsb_multistage_mux_map, e_from_type::FT_PB);

        //biuld multistage mux
        edge_count += build_multistage_mux_for_gsb(x_coord, y_coord, rr_edges_to_create, gsb_multistage_mux_map);
    }
    else{
        VTR_ASSERT(gsb_two_stage_mux_map != nullptr);
        edge_count += build_two_stage_mux_for_gsb(x_coord, y_coord, rr_edges_to_create, gsb_two_stage_mux_map);
    }



    //imux medium to clb_pin
    edge_count += get_medium_to_pin_for_imux(x_coord, y_coord, record_imux_coord, grid, L_rr_node_indices, L_medium_rr_node_indices,
                                             rr_edges_to_create, clbpin_to_medium, other_pbpin_to_clbpin_map, actual_pbtype_name, delayless_switch);
                                             
    //omux
    //edge_count += get_pb_to_omux(x_coord, y_coord, grid, L_rr_node_indices, L_medium_rr_node_indices, rr_edges_to_create, omux_conn_map, actual_pbtype_name, other_pbpin_to_clbpin_map);
 ...
 * vector *///addered by shikc in 200103
static vtr::NdMatrix<std::vector<int>, 5> alloc_and_load_pin_to_track_map(const e_pin_type pin_type,
                                                                          const vtr::Matrix1<int>& Fc,
                                                                          const t_type_ptr Type,
                                                                          const std::vector<bool>& perturb_switch_pattern,
    ...
    for (int iseg = 0; iseg < num_seg_types; iseg++) {
        ...
        for (int pin_index = 0; pin_index < Type->num_pins; ++pin_index)
            for(int i=0; i<4; i++)
            {
            	...
            	if (Fc[pin_index][i][iseg] > max_Fc && Type->class_inf[pin_class].type == pin_type) {
                	max_Fc = Fc[pin_index][i][iseg];
            	}
        	}
    ...
    for (int iseg = 0; iseg < num_seg_types; iseg++) {
        ...
        /* determine the maximum Fc to this segment type across all pins */ //modified by shikc in 200207
        int max_Fc = 0;
        for (int pin_index = 0; pin_index < Type->num_pins; ++pin_index)
        	for(int i=0; i<4; i++){
            	int pin_class = Type->pin_class[pin_index];
         ...
         * as a whole *///modified by shikc in 200219
                    ...
                    for (int iside = 0; iside < 4; iside++) {
                        for(int idx = 0; idx < 4; idx++) {
                            for (int iconn = 0; iconn < max_Fc; iconn++) {
                                int relative_track_ind = pin_to_seg_type_map[ipin][iwidth][iheight][iside][idx][iconn];
                                ...
                                result[ipin][iwidth][iheight][iside][idx].push_back(absolute_track_ind);
                            }
                        }
                    }
                }
...
static void load_uniform_connection_block_pattern(vtr::NdMatrix<int, 6>& tracks_connected_to_pin,
    ...
    for (int i = 0; i < num_phys_pins; ++i) {
        ...
        for (int j = 0; j < (Fc / group_size); ++j) {
            ...
            for (int k = 0; k < group_size; ++k) {
                tracks_connected_to_pin[pin][width][height][side][idx][group_size * j + k] = (itrack + k) % max_chan_width;
...
static vtr::NdMatrix<std::vector<int>, 5> alloc_and_load_track_to_pin_lookup(vtr::NdMatrix<std::vector<int>, 5> pin_to_track_map,
    ...
    auto track_to_pin_lookup = vtr::NdMatrix<std::vector<int>, 5>({size_t(max_chan_width), size_t(type_width), size_t(type_height), 4, 4});
    ...
    for (int pin = 0; pin < num_pins; ++pin) {
        for (int width = 0; width < type_width; ++width) {
            for (int height = 0; height < type_height; ++height) {
                for (int side = 0; side < 4; ++side) {
                    for (int idx = 0; idx < 4; ++idx) {
                        if (pin_to_track_map[pin][width][height][side][idx].empty())
                            continue;

                        /* get number of tracks to which this pin connects */
                        int num_tracks = 0;
                        for (int iseg = 0; iseg < num_seg_types; iseg++) {
                            num_tracks += Fc[pin][idx][iseg];
                        }

                        for (int conn = 0; conn < num_tracks; ++conn) {
                            int track = pin_to_track_map[pin][width][height][side][idx][conn];
                            VTR_ASSERT(track < max_chan_width);
                            VTR_ASSERT(track >= 0);
                            track_to_pin_lookup[track][width][height][side][idx].push_back(pin);
                        }
                    }
                }
            }
...
static void build_unidir_rr_opins(const int i, const int j, const e_side side, const DeviceGrid& grid, const std::vector<vtr::Matrix1<int>>& Fc_out, const int max_chan_width, const t_chan_details& chan_details_x, const t_chan_details& chan_details_y, vtr::NdMatrix<int, 4>& Fc_xofs, vtr::NdMatrix<int, 4>& Fc_yofs, t_rr_edge_info_set& rr_edges_to_create, bool* Fc_clipped, const t_rr_node_indices& L_rr_node_indices, const std::vector<t_rr_node>& rr_nodes, const t_direct_inf* directs, const int num_directs, const t_clb_to_clb_directs* clb_to_clb_directs, const int num_seg_types) {
    /*
    ...
    for (int pin_index = 0; pin_index < type->num_pins; ++pin_index) {
        ...
        for (int idx = 0; idx < 2; idx++) {
            for (int iseg = 0; iseg < num_seg_types; iseg++) {
                ...
                int seg_type_Fc = Fc_out[type->index][pin_index][idx][iseg];
             ...
             * side is the side of the logic or io block. */
                ...
                vtr::NdMatrix<int, 4>& Fc_ofs = (vert ? Fc_xofs : Fc_yofs);
                if (false == pos_dir) {
                    --chan;
                }
                //addered by shikc in 200221
                ...
                if (seg > int(vert ? grid.width() : grid.height()) - 1) { //-2 since no channels around perim
                    continue;
                }
                ...
                if(idx == 0) {
                    bool clipped;
                    if(seg < int(vert ? grid.width() : grid.height()) - 1) {
                        ...
                        if (seg_details[0].length() != 0) {
                            ...
                            get_unidir_opin_connections_inc(chan, seg,
                    ...
                    if(seg < 2) continue;
                    const t_chan_seg_details* seg_details1 = (chan_type == CHANX ? chan_details_x[seg - 1][chan] : chan_details_y[chan][seg - 1]).data();
                    if (seg_details1[0].length() == 0)
                        continue;

                    get_unidir_opin_connections_dec(chan, seg,
                                                    seg_type_Fc, iseg, chan_type, seg_details1,
                                                    opin_node_index,
                                                    rr_edges_to_create,
                                                    Fc_ofs, max_len, max_chan_width,
                                                    L_rr_node_indices, &clipped, grid);
                    ...
                    chan_type = (vert ? CHANY : CHANX);
                    chan = (vert ? (i) : (j));
                    seg = (vert ? (j) : (i));
                    max_len = (vert ? grid.height() : grid.width());
                    Fc_ofs = (vert ? Fc_yofs : Fc_xofs);
...
static void build_unidir_rr_opins_ipins(const int i,
    ...
    for (int pin_index = 0; pin_index < type->num_pins; ++pin_index) {
        ...
        for (int idx = 0; idx < 3; ++idx) {
            ...
            if(block_ipin[i1][j1][side1].empty()) continue;
            int ipin = block_ipin[i1][j1][side1][0];
            std::vector<int>::iterator k = block_ipin[i1][j1][side1].begin();
            block_ipin[i1][j1][side1].erase(k);
            if (ipin >= 0) {
                rr_edges_to_create.emplace_back(opin_node_index, ipin, wire_to_ipin_switch);
                ++num_conn;
            }
        }
    }
...
static t_rr_type find_pt_type(char* type){
    if(strcmp(type,"opin")==0){
        return OPIN;
    }
    else if(strcmp(type,"ipin")==0){
        return IPIN;
    }
    else if(strcmp(type,"chanx")==0){
        return CHANX;
    }
...
static t_pt_to_pt_detailICs* alloc_and_load_pt_to_pt_detailICs(const t_detailInterConnect_inf* detailICs, 
 ...
 {
    int i, j, k;
    t_pt_to_pt_detailICs* pt_to_pt_detailICs;
    char *pb_type_name, *port_name;
    ...
    pt_to_pt_detailICs = (t_pt_to_pt_detailICs*)vtr::calloc(num_detailICs, sizeof(t_pt_to_pt_detailICs));

    pb_type_name = nullptr;
    port_name = nullptr;

    for (i = 0; i < num_detailICs; i++) {
        pb_type_name = (char*)vtr::malloc((strlen(detailICs[i].from_pt) + strlen(detailICs[i].to_pt)) * sizeof(char));
        port_name = (char*)vtr::malloc((strlen(detailICs[i].from_pt) + strlen(detailICs[i].to_pt)) * sizeof(char));
        
        //类型转换，判断chan还是pin
        from_type = find_pt_type(detailICs[i].from_type);
        to_type = find_pt_type(detailICs[i].to_type);
        VTR_ASSERT(from_type != NUM_RR_TYPES);
        VTR_ASSERT(to_type != NUM_RR_TYPES);

        if(from_type==IPIN || to_type==OPIN){
            vpr_throw(VPR_ERROR_ARCH, get_arch_file_name(), detailICs[i].line, "pt type %s and %s don't match.\n", from_type, to_type);
        }

        pt_to_pt_detailICs[i].from_rr_type = from_type;
        pt_to_pt_detailICs[i].to_rr_type = to_type;

        //start from
        if (!(detailICs[i].from.x > 0 
                && detailICs[i].from.x < int(device_ctx.grid.width()-1) 
                && detailICs[i].from.y > 0
                && detailICs[i].from.y < int(device_ctx.grid.height()-1))){
                    vpr_throw(VPR_ERROR_ARCH, get_arch_file_name(), detailICs[i].line, "location (%d ,%d) is out of range!.\n", detailICs[i].from.x, detailICs[i].from.y);
                }
        if (from_type == OPIN) {
            pt_to_pt_detailICs[i].from_clb_type = device_ctx.grid[detailICs[i].from.x][detailICs[i].from.y].type;
            if (detailICs[i].from.z > pt_to_pt_detailICs[i].from_clb_type->capacity){
                vpr_throw(VPR_ERROR_ARCH, get_arch_file_name(), detailICs[i].line, "z(%d) is out of range!.\n", detailICs[i].from.z);
            }
            //pt_to_pt_detailICs[i].to_clb_type = device_ctx.grid[detailICs[i].to.x][detailICs[i].to.y].type;
            // Load from pins
            // Parse out the pb_type name, port name, and pin range
            parse_direct_pin_name(detailICs[i].from_pt, detailICs[i].line, &start_pt_index, &end_pt_index, pb_type_name, port_name);

            // Figure out which type, port, and pin is used
            if (strcmp( pt_to_pt_detailICs[i].from_clb_type->name, pb_type_name) != 0) {
                vpr_throw(VPR_ERROR_ARCH, get_arch_file_name(), detailICs[i].line, "block %s %s don't match actual type at location.\n", pb_type_name, pt_to_pt_detailICs[i].from_clb_type->name);
            }

            pb_type = pt_to_pt_detailICs[i].from_clb_type->pb_type;
            ...
            if (j >= pb_type->num_ports) {
                vpr_throw(VPR_ERROR_ARCH, get_arch_file_name(), detailICs[i].line, "Unable to find port %s (on block %s).\n", port_name, pb_type_name);
            }

            if (start_pt_index == OPEN) {
                VTR_ASSERT(start_pt_index == end_pt_index);
                start_pt_index = 0;
                end_pt_index = pb_type->ports[j].num_pins - 1;
            }
            get_blk_pin_from_port_pin(pt_to_pt_detailICs[i].from_clb_type->index, j, start_pt_index, &pt_to_pt_detailICs[i].from_pt_start_index);
            get_blk_pin_from_port_pin(pt_to_pt_detailICs[i].from_clb_type->index, j, end_pt_index, &pt_to_pt_detailICs[i].from_pt_end_index);
        }
        else {
            pt_to_pt_detailICs[i].from_clb_type = nullptr;

            for (j = 0; j < segment_inf.size(); j++){
                if (strcmp(detailICs[i].from_pt, segment_inf[j].name.c_str()) == 0){
                    break;
                }
            }

            if (j >= segment_inf.size()){
                vpr_throw(VPR_ERROR_ARCH, get_arch_file_name(), detailICs[i].line, "Unable to find seg %s.\n", detailICs[i].from_pt);
            }

            int tmp = detailICs[i].from.tn;
            for (k = 0; k < j; k++){
                tmp += nums_per_segtype[k];
            }

            int max_num = from_type == CHANX? nodes_per_chan->x_list[detailICs[i].from.y] : nodes_per_chan->y_list[detailICs[i].from.x];
            if ( tmp > max_num){
                vpr_throw(VPR_ERROR_ARCH, get_arch_file_name(), detailICs[i].line, "from_tn %s is too large!.\n", detailICs[i].from.tn);
            }
            
            const char* trackName = from_type == CHANX ? 
                                        chan_details_x[detailICs[i].from.x][detailICs[i].from.y][tmp].type_name().c_str():
                                        chan_details_y[detailICs[i].from.x][detailICs[i].from.y][tmp].type_name().c_str();
            if (0 != strcmp(trackName, detailICs[i].from_pt)) {
                vpr_throw(VPR_ERROR_ARCH, get_arch_file_name(), detailICs[i].line, "Error: seg should be %s but we find %s!.\n", detailICs[i].from_pt, trackName);
            }
            //输出的track可以是任一段
            pt_to_pt_detailICs[i].from_pt_start_index = tmp;
            pt_to_pt_detailICs[i].from_pt_end_index = tmp;
            
        }


        //start to
        if (!(detailICs[i].to.x > 0 
                && detailICs[i].to.x < int(device_ctx.grid.width()-1) 
                && detailICs[i].to.y > 0
                && detailICs[i].to.y < int(device_ctx.grid.height()-1))){
                    vpr_throw(VPR_ERROR_ARCH, get_arch_file_name(), detailICs[i].line, "location (%d ,%d) is out of range!.\n", detailICs[i].to.x, detailICs[i].to.y);
                }
        if (to_type == IPIN) {
            pt_to_pt_detailICs[i].to_clb_type = device_ctx.grid[detailICs[i].to.x][detailICs[i].to.y].type;
            if (detailICs[i].to.z > pt_to_pt_detailICs[i].to_clb_type->capacity){
                vpr_throw(VPR_ERROR_ARCH, get_arch_file_name(), detailICs[i].line, "z(%d) is out of range!.\n", detailICs[i].to.z);
            }
            
            // Load to pins
            // Parse out the pb_type name, port name, and pin range
            parse_direct_pin_name(detailICs[i].to_pt, detailICs[i].line, &start_pt_index, &end_pt_index, pb_type_name, port_name);

            // Figure out which type, port, and pin is used
            if (strcmp( pt_to_pt_detailICs[i].to_clb_type->name, pb_type_name) != 0) {
                vpr_throw(VPR_ERROR_ARCH, get_arch_file_name(), detailICs[i].line, "block %s don't match actual type at location.\n", pb_type_name);
            }

            pb_type = pt_to_pt_detailICs[i].to_clb_type->pb_type;
            ...
            get_blk_pin_from_port_pin(pt_to_pt_detailICs[i].to_clb_type->index, j, start_pt_index, &pt_to_pt_detailICs[i].to_pt_start_index);
            get_blk_pin_from_port_pin(pt_to_pt_detailICs[i].to_clb_type->index, j, end_pt_index, &pt_to_pt_detailICs[i].to_pt_end_index);

            if (abs(pt_to_pt_detailICs[i].from_pt_start_index - pt_to_pt_detailICs[i].from_pt_end_index) != abs(pt_to_pt_detailICs[i].to_pt_start_index - pt_to_pt_detailICs[i].to_pt_end_index)) {
                vpr_throw(VPR_ERROR_ARCH, get_arch_file_name(), detailICs[i].line,
                          "Range mismatch from %s to %s.\n", detailICs[i].from_pt, detailICs[i].to_pt);
            }
        }
        else {
            pt_to_pt_detailICs[i].to_clb_type = nullptr;

            for (j = 0; j < segment_inf.size(); j++){
                if(strcmp(detailICs[i].to_pt, segment_inf[j].name.c_str()) == 0){
                    break;
                }
            }
            ...
            int tmp = detailICs[i].to.tn;
            for (k = 0; k < j; k++){
                tmp += nums_per_segtype[k];
            }

            int max_num = to_type == CHANX? nodes_per_chan->x_list[detailICs[i].to.y] : nodes_per_chan->y_list[detailICs[i].to.x];
            if ( tmp > max_num){
                vpr_throw(VPR_ERROR_ARCH, get_arch_file_name(), detailICs[i].line, "from_tn %s is too large!.\n", detailICs[i].to.tn);
            }
            
            const char* trackName = to_type == CHANX ? 
                                        chan_details_x[detailICs[i].to.x][detailICs[i].to.y][tmp].type_name().c_str():
                                        chan_details_y[detailICs[i].to.x][detailICs[i].to.y][tmp].type_name().c_str();
            if (0 != strcmp(trackName, detailICs[i].to_pt)) {
                vpr_throw(VPR_ERROR_ARCH, get_arch_file_name(), detailICs[i].line, "Error: seg should be %s but we find %s!.\n", detailICs[i].to_pt, trackName);
            }

            //输入的track必须是start
            bool start_check_pass = false;
            const t_chan_seg_details& chan_seg_detail = to_type == CHANX ?
                                        chan_details_x[detailICs[i].to.x][detailICs[i].to.y][tmp]:
                                        chan_details_y[detailICs[i].to.x][detailICs[i].to.y][tmp];
            int actual_start = to_type == CHANX ? detailICs[i].to.x : detailICs[i].to.y;
            if (chan_seg_detail.direction() == INC_DIRECTION){
                start_check_pass = chan_seg_detail.seg_start() == actual_start;
            }
            else if (chan_seg_detail.direction() == DEC_DIRECTION){
                start_check_pass = chan_seg_detail.seg_end() == actual_start;
            }
            else if (chan_seg_detail.direction() == BI_DIRECTION){
                start_check_pass = chan_seg_detail.seg_start() == actual_start || chan_seg_detail.seg_end() == actual_start;//shuangxiangxianhaiyaokaokv???
            }


            //int seg_startx=chan_details_x[detailICs[i].to.x][detailICs[i].to.y][tmp].seg_start();
            //int seg_starty=chan_details_y[detailICs[i].to.x][detailICs[i].to.y][tmp].seg_start();
            if ( start_check_pass) {
                pt_to_pt_detailICs[i].to_pt_start_index = tmp;
                pt_to_pt_detailICs[i].to_pt_end_index = tmp;
            }
            else {
                vpr_throw(VPR_ERROR_ARCH, get_arch_file_name(), detailICs[i].line, "Error: to seg %s doesn't start at this loction!.\n", detailICs[i].to_pt);
            }
        }

        //check number match
        if (abs(pt_to_pt_detailICs[i].from_pt_end_index - pt_to_pt_detailICs[i].from_pt_start_index)
            != abs(pt_to_pt_detailICs[i].to_pt_end_index - pt_to_pt_detailICs[i].to_pt_start_index)){
            vpr_throw(VPR_ERROR_ARCH, get_arch_file_name(), detailICs[i].line, "Error: from_num and to_num don't match!!.\n", detailICs[i].to_pt);
        }

        //Set the switch index
        if (detailICs[i].switch_type > 0) {
            //Use the specified switch
            pt_to_pt_detailICs[i].switch_index = detailICs[i].switch_type;
        } else {
            //Use the delayless switch by default
            pt_to_pt_detailICs[i].switch_index = delayless_switch;
        }
        free(pb_type_name);
        free(port_name);
...
static int get_detailICs_connections(t_rr_edge_info_set& rr_edges_to_create,
    ...
    for (i =0; i < num_detailICs; i++) {
        from_x = detailICs[i].from.x;
        from_y = detailICs[i].from.y;
        from_z = detailICs[i].from.z;

        to_x = detailICs[i].to.x;
        to_y = detailICs[i].to.y;
        to_z = detailICs[i].to.z;

        VTR_ASSERT(abs(pt_to_pt_detailICs[i].from_pt_end_index - pt_to_pt_detailICs[i].from_pt_start_index) == abs(pt_to_pt_detailICs[i].to_pt_end_index - pt_to_pt_detailICs[i].to_pt_start_index));
        
        if (pt_to_pt_detailICs[i].from_rr_type == OPIN && pt_to_pt_detailICs[i].to_rr_type == IPIN){
            int total_nums = abs(pt_to_pt_detailICs[i].from_pt_end_index - pt_to_pt_detailICs[i].from_pt_start_index) + 1;
            
            t_type_ptr curr_from_type = device_ctx.grid[from_x][from_y].type;
            t_type_ptr curr_to_type = device_ctx.grid[to_x][to_y].type;
            VTR_ASSERT(pt_to_pt_detailICs[i].from_clb_type == curr_from_type);
            VTR_ASSERT(pt_to_pt_detailICs[i].to_clb_type == curr_to_type);

            int max_index = OPEN, min_index = OPEN;
            bool swap = false;
            int opin, ipin;
            int logical_ipin;
            if (pt_to_pt_detailICs[i].from_pt_start_index > pt_to_pt_detailICs[i].from_pt_end_index) {
                swap = true;
                max_index = pt_to_pt_detailICs[i].from_pt_start_index;
                min_index = pt_to_pt_detailICs[i].from_pt_end_index;
            } else {
                swap = false;
                min_index = pt_to_pt_detailICs[i].from_pt_start_index;
                max_index = pt_to_pt_detailICs[i].from_pt_end_index;
            }
            

            for (j = 0; j < total_nums; j++){
                VTR_ASSERT( min_index + j < curr_to_type->num_pins / curr_to_type->capacity);
                opin = min_index + j + (from_z - 1) * (curr_from_type->num_pins / curr_from_type->capacity);

            
                if (pt_to_pt_detailICs[i].to_pt_start_index > pt_to_pt_detailICs[i].to_pt_end_index) {
                    if (swap) {
                        logical_ipin = pt_to_pt_detailICs[i].to_pt_end_index + j;
                    } else {
                        logical_ipin = pt_to_pt_detailICs[i].to_pt_start_index - j;
                    }
                } else {
                    if (swap) {
                        logical_ipin = pt_to_pt_detailICs[i].to_pt_end_index - j;
                ...
                ipin = logical_ipin + (to_z - 1) * (curr_to_type->num_pins / curr_to_type->capacity);

                
                int width_offset = device_ctx.grid[from_x][from_y].width_offset;
                int height_offset = device_ctx.grid[from_x][from_y].height_offset;

                for (e_side side : SIDES){
                    if (detailICs[i].from.side != NUM_SIDES && detailICs[i].from.side != side){
                        continue;
                    }
                    if (!curr_from_type->pinloc[width_offset][height_offset][side][opin]) {
                        continue; //No source pin on this side
                    }

                    int from_rr_node = get_rr_node_index(L_rr_node_indices, from_x, from_y,
                                                          OPIN, opin, side);
                    if( from_rr_node == OPEN){
                        continue;
                    }
                    std::vector<int> inodes;
                    if (detailICs[i].to.side != NUM_SIDES) {
                        //Explicit side specified, only create if pin exists on that side
                        int inode = get_rr_node_index(L_rr_node_indices, to_x, to_y,
                                                          IPIN, ipin, detailICs[i].to.side);
                        if (inode != OPEN) {
                            inodes.push_back(inode);
                        }
                    } else {
                            //No side specified, get all candidates
                            inodes = get_rr_node_indices(L_rr_node_indices, to_x, to_y,
                                                         IPIN, ipin);
                    }
                    ...
                    if (inodes.size() > 0) {
                        ...
                        rr_edges_to_create.emplace_back(from_rr_node, inode, pt_to_pt_detailICs[i].switch_index);
                        ++num_pins;
                    }
                }
            }
        }
        else if (pt_to_pt_detailICs[i].from_rr_type == OPIN && pt_to_pt_detailICs[i].to_rr_type != IPIN) {
            VTR_ASSERT(pt_to_pt_detailICs[i].from_pt_start_index == pt_to_pt_detailICs[i].from_pt_end_index);
            
            t_type_ptr curr_from_type = device_ctx.grid[from_x][from_y].type;
            VTR_ASSERT(pt_to_pt_detailICs[i].from_clb_type == curr_from_type);

            
            int opin = pt_to_pt_detailICs[i].from_pt_start_index + (from_z - 1) * (curr_from_type->num_pins / curr_from_type->capacity);
            int itrack = pt_to_pt_detailICs[i].to_pt_start_index;
            int width_offset = device_ctx.grid[from_x][from_y].width_offset;
            int height_offset = device_ctx.grid[from_x][from_y].height_offset;
            ...
            for (e_side side : SIDES){
                ...
                int to_rr_node = get_rr_node_index(L_rr_node_indices, to_x, to_y,
                                                    pt_to_pt_detailICs[i].to_rr_type, itrack, NUM_SIDES);
                if( to_rr_node == OPEN){
                    continue;
                }

                rr_edges_to_create.emplace_back(from_rr_node, to_rr_node, pt_to_pt_detailICs[i].switch_index);
                 ++num_pins;
            }  
        }
        else if (pt_to_pt_detailICs[i].from_rr_type != OPIN && pt_to_pt_detailICs[i].to_rr_type == IPIN) {
            VTR_ASSERT(pt_to_pt_detailICs[i].to_pt_start_index == pt_to_pt_detailICs[i].to_pt_end_index);
            
            t_type_ptr curr_to_type = device_ctx.grid[to_x][to_y].type;
            VTR_ASSERT(pt_to_pt_detailICs[i].to_clb_type == curr_to_type);

            int otrack = pt_to_pt_detailICs[i].from_pt_start_index;
            int ipin = pt_to_pt_detailICs[i].to_pt_start_index + (to_z -1)* (curr_to_type->num_pins / curr_to_type->capacity);


            int from_rr_node = get_rr_node_index(L_rr_node_indices, from_x, from_y,
                                                    pt_to_pt_detailICs[i].from_rr_type, otrack, NUM_SIDES);
            if( from_rr_node == OPEN){
                continue;
            }
            ...
            if (inodes.size() > 0) {
                        ...
                        //back fairly directly to the architecture file in the case of pin equivalence
                //int inode = pick_best_direct_connect_target_rr_node(rr_nodes, from_rr_node, inodes);
                //直接选第一个，找最好的函数是对opin而言，找最近的，我这边直接选第一个
                int inode = inodes[0];
                rr_edges_to_create.emplace_back(from_rr_node, inode, pt_to_pt_detailICs[i].switch_index);
                ++num_pins;
            }
