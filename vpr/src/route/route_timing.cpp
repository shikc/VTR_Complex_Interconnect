...
bool try_timing_driven_route(const t_router_opts& router_opts,
    ...
    // used to debug route
    //FILE *route_fp ;
    //route_fp = fopen("route_debug2.txt", "w");
    //FILE* net_delay_fp;
    //net_delay_fp = fopen("net_delay_infos.txt", "w");
    ...
    if (routing_is_successful) {
        ...
        if (timing_info) {
            ...
            // used to output critical path`
            /*FILE* crit_fp;
            crit_fp = fopen("critical_path_infos.txt", "w");
            print_critical_path_info(crit_fp, timing_info);
            fclose(crit_fp);*/
        }
...
t_heap* timing_driven_route_connection_from_route_tree(t_rt_node* rt_root,
    ...
    if (cheapest == nullptr) {
        ...
        VTR_LOG("No routing path for connection to sink_rr %d, retrying with full device bounding box\n", sink_node);
...
static t_heap* timing_driven_route_connection_from_heap(int sink_node,
    ...
    auto& device_ctx = g_vpr_ctx.device();
    fstream fp, fp2;
    //fp.open("path.txt", ios::app);
    if(verbose) {
        fp2.open("delta_path.txt", ios::app);
    }
    while (!is_empty_heap()) {
        ...
        if(verbose) {
            node_vec.push_back(inode);
            if (count == 0) {
                fp2 << "\t\t<node";
                fp2 << " id=\"" << inode;
                fp2 << "\" type=\"" << node.type_string();
                if (node.type() == CHANX || node.type() == CHANY) {
                    fp2 << "\" direction=\"" << node.direction_string();
                }
                fp2 << "\" capacity=\"" << node.capacity();
                fp2 << "\">" << endl;
                fp2 << "\t\t\t<loc";
                fp2 << " xlow=\"" << node.xlow();
                fp2 << "\" ylow=\"" << node.ylow();
                fp2 << "\" xhigh=\"" << node.xhigh();
                fp2 << "\" yhigh=\"" << node.yhigh();
                if (node.type() == IPIN || node.type() == OPIN) {
                    fp2 << "\" side=\"" << node.side_string();
                }
                fp2 << "\" ptc=\"" << node.ptc_num();
                fp2 << "\"/>" << endl;

                if (device_ctx.rr_indexed_data[node.cost_index()].seg_index != -1) {
                    fp2 << "\t\t\t<segment segment_id=\"" << device_ctx.rr_indexed_data[node.cost_index()].seg_index << "\"/>" << endl;
                }
            }
/*
        fp << "\t\t<node";
        ...
        timing_driven_expand_cheapest(cheapest,
                                      ...
                                      router_stats);
        //count++;
        //VTR_LOG("1111111111111111111111************************************\n");
    ...
    if(verbose) {
        /*if(count > 10){
            fp << "count = " << count << "******************" << endl;
            for(int in : node_vec){
                auto& node = device_ctx.rr_nodes[in];
                fp << "\t\t<node";
                fp << " id=\"" << in;
                fp << "\" type=\"" << node.type_string();
                if (node.type() == CHANX || node.type() == CHANY) {
    ...
    if (cheapest == nullptr) { /* Impossible routing.  No path for net. */
        //VTR_LOG("  Empty heap (no path found)\n");
        VTR_LOGV_DEBUG(f_router_debug, "  Empty heap (no path found)\n");
        return nullptr;
    }
    //VTR_LOG("22222222222222222222222222************************************\n");
    return cheapest;
}
...
static t_rt_node* setup_routing_resources(int itry,
    ...
    if ((int)num_sinks < min_incremental_reroute_fanout || itry == 1) {
        ...
        /*auto& route_ctx = g_vpr_ctx.routing();
        if(!verify_traceback_route_tree_equivalent(route_ctx.trace[net_id].head,rt_root)){
            VTR_LOG("error match rt_root and tracback");
        }*/
...
static void evaluate_timing_driven_node_costs(t_heap* to,
    ...
    //VTR_LOG("to_node=%d,target_node=%d,cost=%f\n", to_node, target_node, expected_cost);
