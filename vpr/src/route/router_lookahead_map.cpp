...
static void set_lookahead_map_costs(int segment_index, e_rr_type chan_type, t_routing_cost_map& routing_cost_map, int imux_medium_seg_index, int x_limit, int y_limit);
...
static void get_xy_deltas_gsb(int from_node_ind, int to_node_ind, int* delta_x, int* delta_y);
...
float get_lookahead_map_cost(int from_node_ind, int to_node_ind, float criticality_fac) {
    ...
    //int from_seg_index = device_ctx.rr_indexed_data[from_cost_index].seg_index + from_node.medium_id();

    VTR_ASSERT(from_seg_index >= 0);

    int delta_x, delta_y;
    get_xy_deltas_gsb(from_node_ind, to_node_ind, &delta_x, &delta_y);
...
void compute_router_lookahead(int num_segments, int imux_medium_seg_index) {
    ...
    for (int iseg = 0; iseg < num_segments; iseg++) {
        for (e_rr_type chan_type : {CHANX, CHANY}) {
            ...
            for (int ref_inc = 0; ref_inc < 3; ref_inc++) {
                for (int track_offset = 0; track_offset < MAX_TRACK_OFFSET * fac; track_offset += 2) {
            ...
            if (chan_type == CHANY){
                for (int ref_inc=0; ref_inc<3; ref_inc++){
                    for (int track_offset = 0; track_offset < MAX_TRACK_OFFSET; track_offset += 2){
                        ...
                        int start_node_ind = get_start_node_ind(1, 1+ref_inc,
                                                                device_ctx.grid.width()-2, device_ctx.grid.height()-2,  //non-corner upper right
                                                                chan_type, iseg, track_offset);
                        ...
                        run_dijkstra(start_node_ind, 1, 1+ref_inc, routing_cost_map);
                    }
                }
            }
            ...
            if (chan_type == CHANX){
                for (int ref_inc=0; ref_inc<3; ref_inc++) {
                    for (int track_offset = 0; track_offset < MAX_TRACK_OFFSET; track_offset += 2) {
                        ...
                        int start_node_ind = get_start_node_ind(1 + ref_inc, 1,
                                                                device_ctx.grid.width() - 2,
                                                                device_ctx.grid.height() - 2,  //non-corner upper right
                                                                chan_type, iseg, track_offset);
                        ...
                        run_dijkstra(start_node_ind, 1 + ref_inc, 1, routing_cost_map);
                    }
                }
            }*/
             ...
             * cost map */
            set_lookahead_map_costs(iseg, chan_type, routing_cost_map, imux_medium_seg_index, device_ctx.grid.width()-1 - REF_X, device_ctx.grid.height()-1 - REF_Y);
...
static int get_start_node_ind(int start_x, int start_y, int target_x, int target_y, t_rr_type rr_type, int seg_index, int track_offset) {
    ...
    vector<int> medium_node_list;
    vector<int> medium_num = {device_ctx.medium_seg_inf.gsb_mdeium_num_xory};
    for (int i = 0; i < 1; i++){
        auto vec = device_ctx.medium_rr_node_indices[start_x][start_y][i][0];
        if(rr_type == CHANX){
            medium_node_list.insert(medium_node_list.end(), vec.begin(), vec.begin() + medium_num[i]);
        }
        else{
            medium_node_list.insert(medium_node_list.end(), vec.begin() + medium_num[i], vec.end());
        }
    }

    bool is_normal_chan = false;
    if(start_x == 0 || start_y == 0) return result;
    ...
    if (is_normal_chan) return result;
    //track_offset *= 5;
    for (unsigned itrack = 0; itrack < medium_node_list.size(); itrack++) {
        int node_ind = medium_node_list[itrack];

        //e_direction node_direction = device_ctx.rr_nodes[node_ind].direction();
        int node_cost_ind = device_ctx.rr_nodes[node_ind].cost_index();
        int node_seg_ind = device_ctx.rr_indexed_data[node_cost_ind].seg_index;
        //int node_seg_ind = device_ctx.rr_indexed_data[node_cost_ind].seg_index + device_ctx.rr_nodes[node_ind].medium_id();

        if (node_seg_ind == seg_index) {
            /* found first track that has the specified segment index and goes in the desired direction */
            result = node_ind;
            if (track_offset == 0) {
                break;
    ...
    while (!pq.empty()) {
        ...
        if (device_ctx.rr_nodes[node_ind].type() == IPIN) {
            ...
            if (ipin_x >= start_x && ipin_y >= start_y) {
                int delta_x, delta_y;
                get_xy_deltas_gsb(start_node_ind, node_ind, &delta_x, &delta_y);
...
static void set_lookahead_map_costs(int segment_index, e_rr_type chan_type, t_routing_cost_map& routing_cost_map, int imux_medium_seg_index, int x_limit, int y_limit) {
    ...
    for (unsigned ix = 0; ix < routing_cost_map.dim_size(0); ix++) {
        for (unsigned iy = 0; iy < routing_cost_map.dim_size(1); iy++) {
            ...
            //auto co = expansion_cost_entry.get_representative_cost_entry(REPRESENTATIVE_ENTRY_METHOD);
            //float de = co.delay;
            //float con = co.congestion;
            f_cost_map[chan_index][segment_index][ix][iy] = expansion_cost_entry.get_representative_cost_entry(REPRESENTATIVE_ENTRY_METHOD);
            if(ix <= x_limit && iy <= y_limit && f_cost_map[chan_index][segment_index][ix][iy].delay == -1.0) {
                //if(segment_index == imux_medium_seg_index && ((iy == 0 && ix < 4 && chan_index == 1) || (ix == 0 && iy < 4 && chan_index == 0))){
                /*if(segment_index == imux_medium_seg_index && ((iy == 0 && ix < 4) || (ix == 0 && iy < 4))){
                    Expansion_Cost_Entry& expansion_cost_entry_for_imux = routing_cost_map[ix][iy];
                    f_cost_map[chan_index][imux_medium_seg_index][ix][iy] = expansion_cost_entry_for_imux.get_representative_cost_entry(REPRESENTATIVE_ENTRY_METHOD);
                    continue;
                }*/
                if (!(ix == x_limit && iy == y_limit)) {
                    f_cost_map[chan_index][segment_index][ix][iy] = Cost_Entry(100, 100); //if delay = -1, it usually means no path, so set large cost
                }
            }
        }
    }
...
static void get_xy_deltas(int from_node_ind, int to_node_ind, int* delta_x, int* delta_y) {
    ...
    if(from.R() == 0){//medium_node
        VTR_ASSERT(from_seg_low == from_seg_high);
        delta_seg = abs(to_seg - from_seg_low);
        delta_chan = abs(to_chan - from_chan);
    }
    *delta_x = delta_seg;
    *delta_y = delta_chan;
    if (from.type() == CHANY) {
...
static void get_xy_deltas_gsb(int from_node_ind, int to_node_ind, int* delta_x, int* delta_y) {
    ...
    /* orthogonal to wire */
    delta_chan = abs(from_chan - to_chan);

    /* along same direction as wire. */
    int sb_loc;
    if(from.is_medium_node()){//medium_node
        VTR_ASSERT((from_seg_low == from_seg_high));
        sb_loc = from_seg_low;
    }
    else{
        if(from.direction() == INC_DIRECTION){
            sb_loc = from_seg_high;
        }
        else{
            VTR_ASSERT(from.direction() == DEC_DIRECTION);
            sb_loc = from_seg_low - 1;
        }
    }

    delta_seg = abs(to_seg - sb_loc);
