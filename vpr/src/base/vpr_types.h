...
//    bool is_imux_medium;
    ...
    int cost_index; // for bend segment
    int part_idx;   /* Used for bend segment type */
    enum e_switch_dir_type switch_dir_type;
    int bend_len;
};

struct t_medium_seg_inf {
    //int imux_mdeium_num_xory;
    //int omux_mdeium_num_xory;
    int gsb_mdeium_num_xory;
...
class t_chan_seg_details {
  ...
  public:
    ...
    //bool is_omux_medium() const {return seg_detail_->is_omux_medium;}
    //bool is_imux_medium() const {return seg_detail_->is_imux_medium;}
    bool is_gsb_medium() const {return seg_detail_->is_gsb_medium;}
...
typedef std::vector<std::vector<std::vector<std::vector<std::vector<int>>>>> t_medium_rr_node_indices; //[0..grid_width-1][0..grid_height-1][OMUX/IMUX/GSB][0..num_omuxs/imuxs_types/gsb_types-1][0..pin_num-1]
