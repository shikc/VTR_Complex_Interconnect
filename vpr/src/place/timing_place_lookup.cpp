...
std::unique_ptr<PlaceDelayModel> compute_place_delay_model(t_placer_opts placer_opts,
    ...
    alloc_routing_structs(chan_width, router_opts, arch, det_routing_arch, segment_inf,
                          directs, num_directs, detailICs, num_detailics);
    ...
    if (placer_opts.delay_model_type == PlaceDelayModelType::DELTA
        ...
        route_connection_delay(1,1, 2,1, router_opts, measure_directconnect);
...
static void generic_compute_matrix(vtr::Matrix<std::vector<float>>& matrix,
                                   ...
                                   bool measure_directconnect, 
                                   double& runtime) {
    //VTR_LOG("source_x=%d, source_y=%d, start_x=%d, start_y=%d, end_x=%d, end_y=%d\n", source_x, source_y, start_x, start_y, end_x, end_y);
    //int compute_count = 0;
    int delta_x, delta_y;
    int sink_x, sink_y;
    ...
    for (sink_x = start_x; sink_x <= end_x; sink_x++) {
        ...
        clock_t time_end = clock();
        runtime += 1000 * (time_end - time_start) / (double)CLOCKS_PER_SEC;
        if (runtime > 700000000) {
            cout << "time use:" << runtime << "ms" << endl;
            VTR_LOG("Circuit is unroutable with a channel width factor");
            exit(0);
        }
    }
    //VTR_LOG("compute_count=%d\n",compute_count);
}
