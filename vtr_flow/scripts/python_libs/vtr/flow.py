...
def run(
    ...
    #added by shikc for copying .net and .place file to only run route flow.
    '''f = open("a.txt","w")
    for k,v in vpr_args.items():
        f.write(k)
        f.write(str(v))
        f.write("\n")
    f.close()'''
    if "pack" not in vpr_args:
        netlist_file = str(circuit_file)[:-5]+'.net'
        netlist_copy = str(temp_dir / circuit_file.name)[:-5]+'.net'
        shutil.copy(netlist_file, netlist_copy)

    if "pack" not in vpr_args and "place" not in vpr_args:
        place_file = str(circuit_file)[:-5]+'.place'
        place_copy = str(temp_dir / circuit_file.name)[:-5]+'.place'
        shutil.copy(place_file, place_copy)
